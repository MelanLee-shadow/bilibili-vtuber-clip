...
wsl(ROG-EYE)/Mac 与 free 用同一份 repo、同一套 produce，产出的包是完整的；卡点只
...
- ⑥``authorized_upload make-manifest`` —— **不做**（那是上传授权面，需要 Ivan 的
  逐字引语）。回执里给出该跑的命令，人来接。
...
90 分钟）才写回，任何不持锁的带外手术都会被陈旧写回整体抹掉（2026-08-09 实事故）。
"""
...
def _step_manifest(
    *,
    ...
    result = gate_runner.run(
        [
            sys.executable,
            str(ROOT / "scripts" / "build_lidousha_daily_review_manifest.py"),
...
def _step_title_cover_qc(
    *,
    ...
    # QC 回执的 title_sha256 就与 manifest 不符（2026-08-09 rerun1 实拒）。
    result = gate_runner.run(
        [
            sys.executable,
...
def _step_make_manifest_hint(
    *,
    ...
    receipt.add(
        ...
        "authorized_upload make-manifest binds Ivan's verbatim quote and the "
