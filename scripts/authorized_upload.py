...
``make-manifest`` freezes reviewed bytes and 维护者's authorization; ``upload``
...
def _strict_verified_song_package(
    *,
    ...
    if (
        ...
        or review.get("generated_by") != "build_song_review_manifest.v1"
...
def effective_season_block(manifest: dict) -> tuple[dict | None, str]:
    """(block, provenance).  Legacy manifests (pre-, no season key)
...
def season_add_flow(
    ...
    *,
    ...
    /07-04 incidents: episodes/add wants camelCase ``sectionId`` +
...
def make_manifest(args: argparse.Namespace) -> int:
    ...
    if not args.title.strip() or not args.quote.strip():
        print(
            "REFUSE: --title and --quote (维护者's authorization words) are required non-empty",
            file=sys.stderr,
        )
        return 2
