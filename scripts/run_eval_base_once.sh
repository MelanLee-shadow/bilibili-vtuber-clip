...
#
# 环境变量咒语的唯一权威（2026-07-14 两个实战坑固化于此）：
...
for required in "$BASE/repo/scripts/free_session_autoslice.py" "$BASE/recordings" "$BASE/state"; do
...
exec flock -n "$BASE/runner.lock" env \
  ...
  python3 "$BASE/repo/scripts/free_session_autoslice.py" --once
