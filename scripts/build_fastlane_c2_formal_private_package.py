...
from src.autoslice.fastlane_c2_formal_adapter import CID, NAMES, SCHEMA, TITLE, sha256, visual_inventory


def validate_visual_evidence_source(src: Path) -> list[str]:
 evidence = src / "visual-evidence"
 receipt_path = evidence / "visual-evidence.v1.json"
 if not evidence.is_dir() or evidence.is_symlink() or not receipt_path.is_file() or receipt_path.is_symlink():
  raise SystemExit("missing or unsafe candidate visual evidence")
 try:
  receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
 except (OSError, UnicodeError, json.JSONDecodeError) as exc:
  raise SystemExit(f"invalid candidate visual-evidence receipt: {exc}")
 declared = receipt.get("artifacts")
 if not isinstance(declared, dict) or not declared:
  raise SystemExit("candidate visual-evidence receipt has no artifact inventory")
 expected = set(declared) | {"visual-evidence.v1.json"}
 actual = {path.relative_to(evidence).as_posix() for path in evidence.rglob("*") if path.is_file() and not path.is_symlink()}
 if actual != expected:
  raise SystemExit("candidate visual-evidence inventory is incomplete or has untracked entries")
 for relative, digest in declared.items():
  path = evidence / str(relative)
  if not path.is_file() or path.is_symlink() or sha256(path) != digest:
   raise SystemExit(f"candidate visual-evidence artifact drift: {relative}")
...
def main() -> int:
 ...
 visual_names = validate_visual_evidence_source(src)
 ...
 for relative in visual_names:
  s = src / 'visual-evidence' / relative
  d = out / 'visual-evidence' / relative
  d.parent.mkdir(parents=True, exist_ok=True)
  shutil.copy2(s, d)
  if d.is_symlink() or d.stat().st_size != s.stat().st_size or sha256(d) != sha256(s):
   raise SystemExit(f'visual-evidence copy verification failed: {relative}')
 ...
 manifest={'schema_version':SCHEMA,'candidate_id':CID,'title':TITLE,'upload_allowed':False,'scope':'C2_NAMED_FASTLANE_REPAIR_ONLY','artifacts':artifacts,'visual_evidence_inventory':visual_inventory(out),'record':{'path':'c2.formal.record.v1.json','sha256':sha256(out/'c2.formal.record.v1.json')},'publish':{'path':'c2.formal.publish.v1.json','sha256':sha256(out/'c2.formal.publish.v1.json')}}
