...
# Term-learning loop: 维护者 corrects a name -> edit assets/lidousha/glossary.txt
...
scp -q "$ROOT/scripts/silero_vad_spans.py" "$HOST:/opt/bilive/vad/silero_vad_spans.py"

# 无人值守 runner 的 repo 副本优先于 /opt/bilive/app 回退路径——必须同步更新，
# 否则 runner 用旧资产跑（亲历漂移）。以 profile 清单为边界
