...
维护者 has explicitly reviewed the whole SRT, its typed text-ownership pin lets
...
def _require_operator_authority(value: object, *, label: str) -> dict[str, str]:
    """Accept only a typed 维护者 decision, never a free-form attribution string."""

    if not isinstance(value, dict) or set(value) != {"kind", "evidence_ref"}:
        raise OperatorBaselineCompileError(f"{label} must be typed 维护者 operator authority")
    kind = value.get("kind")
    evidence_ref = value.get("evidence_ref")
    if kind != "REVIEWER_OPERATOR" or not isinstance(evidence_ref, str) or not evidence_ref.strip():
        raise OperatorBaselineCompileError(f"{label} must be typed 维护者 operator authority")
...
def _validate_decision_ledger(
    *,
    ...
    the release SRT.  Only an explicitly typed 维护者 decision can do that.
    """
