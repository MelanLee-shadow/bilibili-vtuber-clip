#!/bin/bash
# Deploy the autoslice runner to free (audit: no more dirty-tree
...
# audit: no more red-suite deploys.
...
# 没有 bypass 开关是故意的——这个仓库里"紧急"应该去找 维护者，不是去找环境变量。
...
PYTHONDONTWRITEBYTECODE=1 /usr/bin/python3 -c \
  "import scripts.session_autoslice; import scripts.produce_slice_package; import src.autoslice.chat_authority"
PYTHONDONTWRITEBYTECODE=1 /opt/bilive/autoslice/venv-diar/bin/python -c \
