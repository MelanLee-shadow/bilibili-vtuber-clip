#!/usr/bin/env python3
"""Compile 维护者 truth-diff v2 into bound delivery baseline artifacts.
...
from scripts.harvest_human_review_truth import parse_srt, split_label
...
REVIEW_STATUS_COMPLETE = "reviewer_reviewed_speaker_complete"
REVIEW_STATUS_PARTIAL_MACHINE = "reviewer_reviewed_speaker_partial_machine"
...
def _strip_render_boundary_separator(
    ...
    *,
    ...
    维护者's annotation format may place one full-width comma at the start of a
...
def _build_delivery_outputs(
    *,
    ...
    override_document = {
        ...
        "notes": (
            "Delivery-time compilation from hash-bound 维护者 truth. Every cue has "
            "operator-reviewed speaker ownership; no machine decision is retained."
            if fully_reviewed
            else (
                "Delivery-time compilation from hash-bound 维护者 truth. Reviewed cues "
            ...
            overrides.append(
                {
                    ...
                    "authority": authority,
                    "note": (
                        "维护者 reviewed speaker truth; mixed segment timing uses "
                        "the established character-proportion proxy and is not acoustic alignment."
                        if len(timed_segments) > 1
                        else "维护者 reviewed speaker truth; exact full-cue interval."
                    ),
