...
def validate_projection(source: list[Cue], projected: list[Cue], authority: dict[str, object]) -> None:
    ...
    if len(inserted) != 1 or not (96_000 <= inserted[0].start_ms <= 98_000 < inserted[0].end_ms):
        raise ValueError("C1 live insertion timing is outside the 01:37 review window")
...
def main() -> int:
    ...
    parser.add_argument("--burn-preview", action="store_true")
    ...
    if args.burn_preview:
        burned = out / f"{CID}.recut.burned-preview.mp4"
        subprocess.run(["ffmpeg", "-hide_banner", "-loglevel", "error", "-nostdin", "-y", "-i", str(out / f"{CID}.recut.mp4"), "-vf", f"ass={out / f'{CID}.recut.final-sapphire72.ass'}", "-map", "0:v:0", "-map", "0:a?", "-c:v", "libx264", "-crf", "18", "-preset", "medium", "-c:a", "aac", "-b:a", "192k", "-movflags", "+faststart", str(burned)], check=True)
