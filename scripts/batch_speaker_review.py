...
GENERATOR_FILES = (
    ...
    ROOT / "src" / "autoslice" / "speaker_common.py",
    ROOT / "src" / "autoslice" / "speaker_context.py",
    ROOT / "src" / "autoslice" / "speaker_evidence.py",
...
def validate_plan(document: object) -> dict[str, Any]:
    ...
    for position, raw in enumerate(entries, start=1):
        ...
        if bool(text_override_path) != expects_override:
            raise BatchSpeakerReviewError(
                f"entry {position} text_authority_mode and operational 维护者 override disagree"
            )
        normalized.append(
            {
                **dict(raw),
