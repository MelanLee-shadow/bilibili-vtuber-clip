...
# real case: ASR is structurally deaf (外套→歪了), only the danmaku recovers it.
_DANMU = "小主的外套是可以脱的吗？"
_GARBLE = "小主歪了可以脱吗"
...
def test_confident_meme_spelling_near_miss_wins_ownership_via_closed_set_witness():
    """主包/主播案 fix: the same confident verdict now falls
...
def test_prompt_and_closed_choice_rules_flag_meme_spelling_and_emote_as_not_typos():
    """Locks the prompt-hint fix: the judge must be told a meme
