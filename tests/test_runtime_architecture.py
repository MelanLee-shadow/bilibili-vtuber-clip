...
#
# 为什么改成账本：这两个断言 随 god-file 拆解一起建立，但只有全局
...
#   - 2026-07-31 之后新增任何一行，rationale 注释必须写明 维护者 的批准出处
...
FUNCTION_DEBT_LEDGER = {
    ...
    # +5：owned_intervals 执法第二个权威源接线（维护者
    ...
    ("src/autoslice/producer_text_finalization.py", "verify_chat_authority_final_surfaces"): 328,
    ...
    # +15：维护者 优化①边界重放 + wsl 重产 BLOCK
    # 实证——loader、source/final frozen verdict 与 carry disclosure 接线；
    # hash/输入锚不符仍走原 fresh reviewer。
    # +9：终审结转硬退出侧车（维护者 15:05Z 交棒清单第 7 项
    # 逐字「硬退出丢 carryover(超时/崩溃跳过侧车落盘)」，内部交接记录）——
    ...
    # +9：显式降级执行位（demotion_detail 形参 + READY_DEGRADED
    # 记录）——维护者 8/2 /goal「全都按你的想法进行修复」授权，封面路由 P1。
    ("src/autoslice/publish_staging.py", "_stage_cpa_redraw_cover"): 418,
    # +2：同上，contract 穿透接线。
    # +29：截图物化失败→显式降级重绘（降级回执+细节留痕，重绘
    ...
    # 下次动这个函数必须先拆，不许再抬。
    # +47：reuse 结转已发布 cover_generation 证据包（1013 r14 案，
    # recovery manifest 要求 route-decision.v2；sha 逐字节相等才结转）。
    # +45：身份见证现场重打（摘要形态回执过不了现行校验，不考古，
    # 对同一字节新打 CPA 见证；失败即丢弃整个结转 fail-closed）。
    # +19：合并 ft-a8600994 快车道分支（维护者 逐字「这就是要
    ...
    # +3：同上，截图/polish 路径的 contract 穿透。
...
MODULE_DEBT_LEDGER = {
    ...
    # +1：维护者 8/8 真值法证 synthesis F2——候选级代词
    ...
    # +14：child_env_for_date 绑定会话游戏语境（维护者 8/7 指令：
    ...
    # +11：维护者 两句逐字——①「把 tier1 的 4 条做了」；
    # ②「87 现在需要纳入处理范围」。早已滑出 list_dates() 的最新三天
    ...
    # 因此回到 300 行以内、未进函数账本。维护者 07-31 `/goal` 授权 + Fable 裁定链。
    # 再 +52：_talk_font_floor_layout_override——无梗字单行文案的
    # 120px 下限版面自愈（维护者 07-31 原话拍板「120px 是硬性要求，无所谓是什么
    # layout，接受版面切换」）。
    ...
    "src/autoslice/cover_repair.py": 2_162,
    ...
    # 函数账本注释里，维护者 本人下达修正指令视为对本次追加的批准出处。测试
    # tests/test_final_review_auditor.py::
    ...
    # 净 -269：F3 三逃生口纯判定抽到 candidate_support.py，
    # 同时接入 维护者 8/8 真值法证 F1 回声环修复；锁定拆解后的模块收益。
    # 再净 -16：同日 truth-harvest synthesis F7 删除无声学
    # context-only mutation 快捷路，保留显式 disclosure 降级门。
    # 净 -77：维护者 F16/F17 指令要求新逻辑放新模块；结构化闭集
    ...
    # +6：owned_intervals 执法调用点接线（维护者 配额上传
    ...
    # +5：维护者 优化①边界重放 + wsl 重产 BLOCK
    # 实证——把两层 carry/skip 披露写入最终 boundary audit。
    # +1：同上那句逐字——交付摘要里加一个 speaker_guess 紧凑指纹
    ...
    # b：与 ft-a8600994 合并后按**实测**行数记（两侧各自记的 2_742 /
    # 2_740 都不是合并后的真实值；账本记实测，不记任一侧的旧快照）。
    # c +39：不可读窗删除车道（维护者 逐字见上面
    ...
    # +10：owned_intervals 执法第二个权威源接线（维护者
    ...
    # +23：维护者 优化①边界重放(8b 冻结 loader 接线)
    # +30：维护者 8/8 真值法证 F2 代词发现器接线;两处主体均在独立新模块。
    # 净 -1：F16/F17 priority assembler 保留旧测试入口并拆分候选计数。
    # 净 -35：F20 真值全所有权快路径（维护者 立项，
...
ENTRY_FILE_LINE_BUDGETS = {
    ...
    # +63 来自 8356710（歌切最终歌词交 CPA 按 hash 绑定重判），
...
FOCUSED_MODULE_LINE_BUDGETS = {
    ...
    # −256：CAM++ 的 embed-once 原语（cosine/嵌入校验/内容寻址缓存
    ...
    # +13：三处并行改动合流后的实际值——(a) worker A 的 max_lines
...
def test_full_text_cover_contract_reaches_every_renderer_call_site() -> None:
    ...
    废除 talk 的 mode=full 自动回退后，hash 绑定的 full-text contract
    ...
    call_sites = {
        "src/autoslice/publish_staging.py": "_overlay_cover_title",
        "src/autoslice/cover_polish_gate.py": "_overlay_cover_title",
    }
    missing: list[str] = []
...
def test_cover_only_lane_reuses_the_proven_video_repair_io_surface() -> None:
    ...
    `same_bv_cover_repair`（`28b3576` 新建）**零生产执行**。它的风险
