"""截图优先、重绘兜底（2026-08-10）的回归金丝雀。

设计稿：``docs/reviews/2026-08-10-cover-route-screenshot-first-forensics.md`` D 节。
六条一一对应：

1. 负向 · 2026-07-26 BV1E93L6rErV「1411 角落小人」——**P1 不许放开的边界**；
2. 负向 · 2026-07-22 空面板游戏 UI——放宽游戏场后仍必须被拦；
...
def _verified(reference: Path, verdict: dict[str, object], **kwargs) -> dict[str, object]:
    verification = verify_lidousha_source_composition(
...
# 2026-07-26 BV1E93L6rErV：她只是右下角小头像，忠实裁切也成不了大主体。
...
def test_canary_1b_final_pixel_gate_still_rejects_a_corner_avatar_cover(tmp_path):
    ...
    try:
        verification = verify_lidousha_final_host_identity(
...
def test_canary_2b_final_game_gate_rejects_an_empty_panel(tmp_path):
    ...
    base = {
        ...
        # 她确实不是主体——这正是 Ivan 8/9 说的"主体肯定是游戏"，不该扣分。
        "primary_subject_is_visually_dominant": False,
    ...
    def _verify(verdict):
        ...
        try:
            return verify_lidousha_final_host_identity(
                final_cover_path=final_cover,
                final_cover_sha256=_sha(final_cover),
                reference_path=reference,
                scene_kind=GAME_SCENE,
    ...
    # 基线：她不占主要部分也能过——这就是 Ivan 8/9 裁定的机器面。
...
def test_canary_6b_game_scene_materializes_the_whole_frame(tmp_path):
    """Ivan 8/9：主体本来就是游戏——不许把小窗 1.38x 裁出来当封面。"""
