...
def test_real_20260807_case_extends_to_reviewer_confirmed_boundary() -> None:
    """真值回归：流水线的 654170 后延到 维护者 复核确认的 734230。"""
    ...
    assert receipt["extended_end_ms"] == REVIEWER_CONFIRMED_END_MS
...
def test_semantic_recall_lane_wires_the_extension(with_danmaku: bool, tmp_path) -> None:
    ...
    expected_end = REVIEWER_CONFIRMED_END_MS if with_danmaku else REAL_PIPELINE_END_MS
...
def test_sharded_recall_also_gets_the_extension(tmp_path) -> None:
    ...
    assert len(calls) > 1
    target = next(c for c in selected if c.boundary.resolved_start_ms == REAL_START_MS)
    assert target.boundary.resolved_end_ms == REVIEWER_CONFIRMED_END_MS
    assert AUDIENCE_PAYOFF_EXTENDED_REASON in target.boundary.reason_codes
    receipts = diagnostics["audience_payoff_extensions"]
    assert [r["extended_end_ms"] for r in receipts] == [REVIEWER_CONFIRMED_END_MS]
