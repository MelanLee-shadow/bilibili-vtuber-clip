...
class FakeBili:
    ...
    def __init__(self, *, state=0, add_code=0):
        ...
        self.calls: list[str] = []
        self._season_by_section = {9110001: "小李切片", 9110002: "小李歌唱"}
        self._season_id_by_section = {9110001: 8110001, 9110002: 8110002}
            ...
            if "web/seasons" in url:
                return {
                    ...
                    "data": {
                        "seasons": [
                            {
                                "season": {"id": 8110001, "title": "小李切片"},
                                "sections": {"sections": [{"id": 9110001, "title": "正片"}]},
                            },
                            {
                                "season": {"id": 8110002, "title": "小李歌唱"},
                                "sections": {"sections": [{"id": 9110002, "title": "正片"}]},
                            },
                        ]
                    },
                }
...
def test_upload_finishes_season_add_and_public_verify(tmp_path, monkeypatch, capsys):
    ...
    assert rc == 0
    assert fake.added_sections[0]["sectionId"] == 9110001  # talk → 小李切片 正片
    ...
    assert any(
        url.endswith("creative/web/season/section?id=9110001")
...
def test_upload_song_manifest_targets_song_season(tmp_path, monkeypatch):
    ...
    assert rc == 0
    assert fake.added_sections[0]["sectionId"] == 9110002  # song → 小李歌唱 正片
...
def test_upload_pending_transcode_exits_6_and_season_add_retries(tmp_path, monkeypatch):
    ...
    assert rc == 0
    assert fake.added_sections[0]["sectionId"] == 9110001
...
def test_legacy_manifest_remains_verify_readable_but_cannot_new_upload(tmp_path):
    ...
    del data["season"]  # 之前的 manifest 没有 season 键
    manifest.write_text(json.dumps(data, ensure_ascii=False), encoding="utf-8")
