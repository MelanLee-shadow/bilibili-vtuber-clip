...
def test_judge_prompt_excludes_cover_songs_from_upload_risk(tmp_path: Path):
    ...
    （冒烟实证：判官曾以"受版权保护的歌曲演唱"为由 BLOCK 掉合格歌切）。"""
...
def test_direct_responses_mode_posts_responses_endpoint(monkeypatch):
    """the judge moved to gpt-5.6-luna — gpt-5.x are Responses-only
...
def test_judge_falls_back_to_second_model_when_primary_rate_limited(tmp_path, monkeypatch):
    """live probe: the whole 5.6 family 429s in one provider usage
    window — the judge must fall back (维护者's standing rule: keep a 5.5
