...
def test_unwitnessed_name_guess_reverts_but_agy_witness_passes():
    """生日结婚实案：BCUT 误听「留下」，AGY 缺席时 CPA 猜成
...
def test_pinyin_homophone_respell_passes_without_witness():
    """五年之约冤杀案回归：季下→记下(jì同音)、小丽→小主(lǐ同音)
        ...
        "overrides": [
            {
                ...
                "authority": "维护者 confirmed this is watched-video audio",
...
def test_structured_chat_name_witnesses_its_own_kana():
    ...
    Live case auto_193450_1573_1672 cue 76: the host reads
...
def test_truth_owned_mixed_latin_cues_defer_to_source_truth():
    """742_887 案：BCUT 乱码「say you say father 哈」的时窗
