...
def test_zero_edit_is_a_complete_valid_review():
    out = transcription._cpa_correct_draft_cues(
        DRAFT, danmaku_lines=[], cpa_llm_call=lambda _: '{"cues":[{"n":1,"text":"不是不是十五"}]}'
    )
    assert out == DRAFT
    pronoun = DRAFT.replace("不是不是十五", "TA说不是")
    assert (
        transcription._cpa_pronoun_ta_pass(
            pronoun, cpa_llm_call=lambda _: '{"rewrites":[]}', required=True
        )
        == pronoun
    )
...
def test_moss_route_keeps_raw_and_requires_cpa(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        free, "transcribe", lambda *a, **k: pytest.fail("MOSS success must not run BCUT")
    )
    monkeypatch.setattr(
        moss_transcription,
        "transcribe_moss",
        lambda audio, **k: (DRAFT, {"provider": "moss", "speaker_labels": ["S01"]}),
    )
    monkeypatch.setattr(
        transcription,
        "_build_ssh_agy_runner",
        lambda *a, **k: lambda *a, **k: pytest.fail("MOSS success must not run full AGY"),
    )
    ...
    assert (
        json.loads(media.with_suffix(".asr-source.json").read_text())["effective_route"]
        == "moss_cpa"
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **k: {
            "provider": "bcut",
            "utterances": [
                {"start_time": 0, "end_time": 1000, "transcript": "不是不是十五", "words": []}
            ],
        },
    )
    attempts = []
    ...
    assert source["effective_route"] == "cpa"
...
def test_runtime_default_cannot_disable_cpa(monkeypatch):
    ...
    with pytest.raises(SystemExit):
        parse_producer_args(
            ["--spec", "unused.json"],
            description="test",
            speaker_display_name="test",
            default_speaker_mode="required",
        )


def test_ordinary_default_reviews_bcut_without_whole_clip_agy(tmp_path, monkeypatch):
    import scripts.free_asr_client as free
    from src.autoslice import llm_client
    from src.autoslice.producer_request import parse_producer_args

    monkeypatch.delenv("AUTOSLICE_CORRECTION_MODE", raising=False)
    ...
    monkeypatch.setattr(
        transcription,
        "_build_ssh_agy_runner",
        lambda *a, **k: pytest.fail("ordinary transcription must not construct whole-clip AGY"),
    )
    calls = []
    ...
    monkeypatch.setattr(llm_client, "build_llm_call", lambda _: review)
    media = tmp_path / "ordinary.mp4"
    media.write_bytes(b"fixture")
    run = transcription._build_aggregate_asr_transcriber("localhost", correct=args.correct)
    assert run(media) == DRAFT
    assert len(calls) == 1
    assert media.with_suffix(".cpa-reviewed.srt").read_text() == DRAFT
    assert not media.with_suffix(".agy_refined.srt").exists()
    source = json.loads(media.with_suffix(".asr-source.json").read_text())
    assert source["effective_route"] == "cpa"
...
@pytest.mark.parametrize(("value", "offset", "expected"), [
    ...
    ("5.9", 2500, "00:03"),
    ("00:05", -1000, "00:06"),
    ("unreadable", 2000, "unreadable"),
])
def test_screen_clock_rebase_retains_transcription_import_seam(value, offset, expected):
    from src.autoslice import full_session_transcription as transcription
    from src.autoslice.subtitle_draft_preparation import _rebase_mmss

    assert transcription._rebase_mmss is _rebase_mmss
    assert _rebase_mmss(value, offset) == expected
