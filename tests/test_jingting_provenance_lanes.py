"""Typed jingting provenance lanes (2026-08-10 歌lane provider门根因修复).

2026-08-07 六条歌切候选的 reason_codes 全部带 ``JINGTING_PROVIDER_NOT_AGY`` +
...
3. 这两个 code 同时是 runner 的 ``SONG_INFRA_TRANSIENT_REASON_CODES`` 成员，
   ...
   （2026-08-08 单场 26 次重试 / 0 产出的僵尸循环）。
...
# scripts/free_session_autoslice.py 的 SONG_INFRA_TRANSIENT_REASON_CODES 中与
# jingting provenance 相关的成员。runner 是 4000+ 行的 __main__ 模块，这里按值
# 复制以免为了一个常量拖入整个 runner；free_session_autoslice.py:316-322 是真值。
...
# 2026-08-07 free 上真实落盘的旁路 manifest（逐字，取自
...
class TestGeminiApiFallbackLane:
    """Ivan 2026-07-19 拍板（项目 memory）：AGY 订阅 / 免费 key / 付费 backup 是
    同一个 Gemini 模型的**配额顺序**，「按 provider 层拒证据的门 = 过度限制」；
    2026-08-10「需要调用 AGY->gemini 这条链的，全都复用一种接口才好」。
...
class TestSongHintSamplingDensity:
    ...
    2026-08-07 实测：同一候选，短窗（~30 cue）识别出歌名，214-cue 的 ``_full``
