...
def _truncated_ignored_fixture(tmp_path: Path) -> tuple[argparse.Namespace, Path, dict, dict]:
    args = _args(tmp_path)
    date_dir = args.record_root / "2026-08-25"
    date_dir.mkdir(parents=True)
    source = date_dir / "123456_20260825-13-07-44.flv"
    source.write_bytes(b"short reconnect fragment")
    source.with_suffix(".xml").write_text(_connection_stub_xml(event_count=0), encoding="utf-8")
    relative = f"{date_dir.name}/{source.name}"
    webhook = {
        "status": "OPEN",
        "session_id": "truncated-session",
        "opening_event_id": "opening-fragment",
    ...
    row = adapter.build_truncated_source_disposition(
        ...
        webhook=webhook,
        xml_recovery={
            "repaired_sha256": adapter.hashlib.sha256(b"repaired").hexdigest(),
    ...
    args.state_path.write_text(json.dumps(state), encoding="utf-8")
    return args, source, state, row


def _truncated_manifest_fixture(
    tmp_path: Path,
    monkeypatch,
    *,
    stage_present: bool = False,
    target_present: bool = False,
) -> tuple[argparse.Namespace, Path, Path, dict, dict, dict, Path]:
    args = _args(tmp_path)
    args.status_path.parent.mkdir(parents=True, exist_ok=True)
    args.offline_confirmed = True
    args.offline_status_max_age = 300.0
    date_dir = args.record_root / "2026-08-25"
    date_dir.mkdir(parents=True)
    source = date_dir / "123456_20260825-13-07-44.flv"
    ...
    xml.write_bytes(
        (
            ...
            '<BililiveRecorderRecordInfo roomid="123456" name="主播" title="测试" '
            'start_time="2026-08-25T13:07:44+08:00"/>X'
        ).encode("utf-8")
    )
    relative = f"{date_dir.name}/{source.name}"
    webhook = {
        ...
        "opening_event_id": "opening-manifest",
        "closing_event_id": "closing-manifest",
        "file_open_time": "2026-08-25T13:07:44+08:00",
        "file_close_time": "2026-08-25T13:27:44+08:00",
        "file_size": source.stat().st_size,
        "duration": 1200.0,
    }
    errors = adapter._truncated_ordinary_errors(source, webhook)
    source_media = {
    ...
    output_bytes = b"recovered-output"
    output_media = {
        "duration_seconds": 1200.0,
    ...
    remux_stderr = "warning at pointer 0xABCD"
    normalized_sha256 = adapter._truncated_normalized_stderr_sha256(remux_stderr)
    calls = {"remux": 0}

    def fake_probe(path: Path, **_kwargs):
        return source_media if Path(path) == source else output_media

    def fake_packets(path: Path, **_kwargs):
        return {"video": 10, "audio": 5} if Path(path) == source else {"video": 8, "audio": 4}

    def fake_remux(_source: Path, stage: Path, **_kwargs):
        calls["remux"] += 1
        stage.parent.mkdir(mode=0o700, exist_ok=True)
        stage.write_bytes(output_bytes)
        return ["ffmpeg", "-fflags", "+genpts+discardcorrupt"], subprocess.CompletedProcess(
            [], 0, "", remux_stderr
        )
    ...
    monkeypatch.setattr(adapter, "probe_media", fake_probe)
    monkeypatch.setattr(adapter, "_truncated_media_packet_counts", fake_packets)
    monkeypatch.setattr(adapter, "_truncated_stream_copy", fake_remux)
    monkeypatch.setattr(adapter, "_truncated_full_decode", fake_decode)

    original_xml = xml.read_bytes()
    repaired_xml = original_xml[:-1] + b"</i>"
    repair = {
        "keep_bytes": len(original_xml) - 1,
        "tail_bytes_discarded": 1,
        "repaired_sha256": hashlib.sha256(repaired_xml).hexdigest(),
        "recovered_event_count": 0,
        "record_info": {
            "roomid": "123456",
            "name": "主播",
            "title": "测试",
            "start_time": "2026-08-25T13:07:44+08:00",
        },
    }
    expected = {
        "source_media": source_media,
        "source_packets": {"video": 10, "audio": 5},
        "output_media": output_media,
        "output_packets": {"video": 8, "audio": 4},
        "output_sha256": hashlib.sha256(output_bytes).hexdigest(),
        "stream_copy_stderr_normalized_sha256": normalized_sha256,
        "full_decode_stderr_sha256": hashlib.sha256(b"").hexdigest(),
    }
    row = {
        "source_relative_path": relative,
        "action": adapter.TRUNCATED_SOURCE_DISPOSITION_RECOVERED,
        "source": {"path": str(source), **adapter._attest_regular_file(source)},
        "xml": {"path": str(xml), **adapter._attest_regular_file(xml)},
        "webhook": webhook,
        "errors": errors,
        "xml_repair": repair,
        "packet_loss_maximum": {"video": 2, "audio": 1},
        "warning": {"normalized_sha256": normalized_sha256, "allowed": True},
        "expected": expected,
    }
    unsigned = {
        "schema_version": adapter.TRUNCATED_SOURCE_MANIFEST_SCHEMA_VERSION,
        "room_id": 123456,
        "rows": [row],
    }
    manifest = {
        **unsigned,
        "canonical_integrity": {
            "algorithm": "sha256",
            "canonical_json_sha256": adapter._canonical_json_sha256(unsigned),
        },
    }
    args.state_path.parent.mkdir(parents=True, exist_ok=True)
    state = {
    ...
    args.state_path.write_text(json.dumps(state), encoding="utf-8")
    os.chmod(args.state_path, 0o600)
    ...
    manifest_path = args.state_path.parent / "truncated-source-manifest.json"
    manifest_path.write_text(json.dumps(manifest), encoding="utf-8")
    os.chmod(manifest_path, 0o600)
    args.apply_truncated_source_manifest = manifest_path
    args.expected_truncated_manifest_sha256 = manifest["canonical_integrity"][
        "canonical_json_sha256"
    ]
    stage = adapter._truncated_stage_path(
        source,
        record_root=args.record_root,
    ...
    if stage_present:
        ...
        stage.write_bytes(output_bytes)
    if target_present:
        source.with_suffix(".mp4").write_bytes(output_bytes)
    return args, source, xml, manifest, expected, calls, stage


def test_truncated_manifest_recovery_is_atomic_and_idempotent(tmp_path: Path, monkeypatch) -> None:
    args, source, xml, _manifest, _expected, calls, _stage = _truncated_manifest_fixture(
        tmp_path, monkeypatch
    )
    original_source, original_xml = source.read_bytes(), xml.read_bytes()
    before_state = args.state_path.read_bytes()
    ...
    assert stat.S_IMODE(args.state_path.stat().st_mode) == 0o600
    state_after = args.state_path.read_bytes()
    assert state_after != before_state
    row = json.loads(state_after)["source_dispositions"][result["rows"][0]["source_relative_path"]]
    assert row["recovery"]["stream_copy"]["stderr_normalized_sha256"] == _manifest["rows"][0]["warning"]["normalized_sha256"]
    assert row["recovery"]["warning"]["sha256"] == hashlib.sha256(
        b"warning at pointer 0xABCD"
    ).hexdigest()

    assert adapter.apply_truncated_source_manifest(args)["recovered"] == 1
    assert calls["remux"] == 1
    assert args.state_path.read_bytes() == state_after


def test_truncated_manifest_stage_resume_does_not_remux(tmp_path: Path, monkeypatch) -> None:
    args, _source, _xml, _manifest, _expected, calls, stage = _truncated_manifest_fixture(
        tmp_path, monkeypatch, stage_present=True
    )
    result = adapter.apply_truncated_source_manifest(args)
    assert result["recovered"] == 1
...
def test_truncated_manifest_target_resume_validates_target(tmp_path: Path, monkeypatch) -> None:
    args, _source, _xml, _manifest, _expected, calls, _stage = _truncated_manifest_fixture(
        tmp_path, monkeypatch, target_present=True
    )
    result = adapter.apply_truncated_source_manifest(args)
    assert result["recovered"] == 1
    assert calls["remux"] == 0
...
def test_truncated_manifest_ignored_fragment_writes_no_outputs(tmp_path: Path, monkeypatch) -> None:
    ...
    expected["source_media"]["duration_seconds"] = 5.0
    row = manifest["rows"][0]
    row["action"] = adapter.TRUNCATED_SOURCE_DISPOSITION_IGNORED
    row.pop("packet_loss_maximum")
    row.pop("warning")
    row.pop("expected")
    unsigned = {key: value for key, value in manifest.items() if key != "canonical_integrity"}
    manifest["canonical_integrity"] = {
        "algorithm": "sha256",
        "canonical_json_sha256": adapter._canonical_json_sha256(unsigned),
    }
    args.apply_truncated_source_manifest.write_text(json.dumps(manifest), encoding="utf-8")
    os.chmod(args.apply_truncated_source_manifest, 0o600)
    args.expected_truncated_manifest_sha256 = manifest["canonical_integrity"][
        "canonical_json_sha256"
    ]
    ...
    assert not source.with_suffix(".jsonl").exists()
    assert not source.with_suffix(".meta.json").exists()
    assert json.loads(args.state_path.read_text())["finalized"] == {}


def test_truncated_manifest_rejects_drift_and_lock_contention(tmp_path: Path, monkeypatch) -> None:
    args, source, _xml, manifest, _expected, _calls, _stage = _truncated_manifest_fixture(
        tmp_path, monkeypatch
    )
    source.write_bytes(b"tampered-source")
    with pytest.raises(adapter.AdapterError, match="manifest source/XML evidence drifted"):
        adapter.apply_truncated_source_manifest(args)

    lock_fd = adapter._truncated_acquire_state_lock(args.state_path)
    try:
        with pytest.raises(adapter.AdapterError, match="owner lock is unavailable"):
            adapter.apply_truncated_source_manifest(args)
    finally:
        adapter._truncated_release_state_lock(lock_fd)
...
def test_truncated_ignored_fragment_is_explicit_and_suppresses_only_exact_open_errors(
    tmp_path: Path, monkeypatch
) -> None:
    args, source, state, row = _truncated_ignored_fixture(tmp_path)
    original_source = source.read_bytes()
    original_xml = source.with_suffix(".xml").read_bytes()
    lookalike = source.with_name("123456_20260825-13-07-45.flv")
    lookalike.write_bytes(b"another fragment")
    lookalike_relative = f"2026-08-25/{lookalike.name}"
    state["webhook_files"][lookalike_relative] = {
        **state["webhook_files"][f"2026-08-25/{source.name}"],
        "opening_event_id": "opening-lookalike",
    }
    args.state_path.write_text(json.dumps(state), encoding="utf-8")
    monkeypatch.setattr(
        adapter,
    ...
    lookalike_errors = [
        entry["error"] for entry in status["finalize_errors"] if entry["source"] == str(lookalike)
    ]
    assert lookalike_errors == [
        "source lacks BililiveRecorder FileClosed evidence",
        "FileOpening has no matching FileClosed event",
    ]
    assert all(entry["source"] != str(source) for entry in status["finalize_errors"])
    assert source.read_bytes() == original_source
    assert source.with_suffix(".xml").read_bytes() == original_xml
    assert row["errors"] == [
        {
...
def test_truncated_errors_recompute_and_source_drift_fail_closed(tmp_path: Path) -> None:
    _args_value, source, state, row = _truncated_ignored_fixture(tmp_path)
    relative = f"2026-08-25/{source.name}"
    assert adapter.revalidate_source_dispositions(
        state,
        record_root=source.parent.parent,
        room_id=123456,
    ) == {relative}

    row["errors"][0]["message"] = "source size does not match BililiveRecorder FileClosed (1 != 2)"
    unsigned = {key: value for key, value in row.items() if key != "canonical_integrity"}
    row["canonical_integrity"]["canonical_json_sha256"] = adapter._canonical_json_sha256(unsigned)
    with pytest.raises(adapter.AdapterError, match="ordinary finalization error projection"):
...
def test_truncated_error_projection_binds_size_numbers_and_xml_parse_message(
    tmp_path: Path,
) -> None:
    args, source, _state, _row = _truncated_ignored_fixture(tmp_path)
    closed = {
        "status": "CLOSED",
        "session_id": "closed-session",
        "opening_event_id": "opening-closed",
        "closing_event_id": "closing-closed",
        "file_open_time": "2026-08-25T13:07:44+08:00",
        "file_close_time": "2026-08-25T13:27:44+08:00",
        "file_size": source.stat().st_size + 17,
        "duration": 1200.0,
    }
    assert adapter._truncated_ordinary_errors(source, closed) == [
        {
            "category": "FILE_CLOSED_SIZE_MISMATCH",
            "message": (
                "source size does not match BililiveRecorder FileClosed "
                f"({source.stat().st_size} != {source.stat().st_size + 17})"
            ),
        }
    ]

    source.with_suffix(".xml").write_bytes(b"<i")
    closed["file_size"] = source.stat().st_size
    parse_errors = adapter._truncated_ordinary_errors(source, closed)
    assert parse_errors == [
        {
            "category": "XML_TAIL_FINALIZATION",
            "message": "invalid BililiveRecorder XML 123456_20260825-13-07-44.xml: unclosed token: line 1, column 0",
        }
    ]
    assert args.record_root == source.parent.parent


def test_truncated_disposition_source_webhook_and_integrity_tamper_fail_closed(
    tmp_path: Path,
) -> None:
    args, source, _state, _row = _truncated_ignored_fixture(tmp_path / "source")
    source.write_bytes(b"X" * source.stat().st_size)
    state = json.loads(args.state_path.read_text(encoding="utf-8"))
    with pytest.raises(adapter.AdapterError, match="source fingerprint drifted"):
        adapter.revalidate_source_dispositions(
            state,
            record_root=args.record_root,
            room_id=123456,
    ...
    args, _source, _state, _row = _truncated_ignored_fixture(tmp_path / "webhook")
    state = json.loads(args.state_path.read_text(encoding="utf-8"))
    relative = next(iter(state["source_dispositions"]))
    state["webhook_files"][relative]["opening_event_id"] = "tampered-opening"
    with pytest.raises(adapter.AdapterError, match="canonical integrity mismatch|webhook projection drifted"):
        adapter.revalidate_source_dispositions(
            state,
            record_root=args.record_root,
            room_id=123456,
    ...
    args, _source, _state, _row = _truncated_ignored_fixture(tmp_path / "integrity")
    state = json.loads(args.state_path.read_text(encoding="utf-8"))
    relative = next(iter(state["source_dispositions"]))
    state["source_dispositions"][relative]["canonical_integrity"]["canonical_json_sha256"] = "0" * 64
    with pytest.raises(adapter.AdapterError, match="canonical integrity mismatch"):
        adapter.revalidate_source_dispositions(
            state,
            record_root=args.record_root,
            room_id=123456,
...
def test_connection_stub_fuse_timestamp_rebinds_successor_mp4_metadata_only(
    tmp_path: Path,
    monkeypatch,
) -> None:
    stub, successor, webhook_files, finalized = _connection_stub_fixture(tmp_path, monkeypatch)
    ...
    assert row is not None
    binding = row["session"]["successor_mp4"]
    binding["mtime_ns"] -= 10
    binding["ctime_ns"] -= 20
    unsigned = {key: value for key, value in row.items() if key != "canonical_integrity"}
    row["canonical_integrity"]["canonical_json_sha256"] = adapter._canonical_json_sha256(
    ...
    assert receipt["schema_version"] == "recording-source-fuse-timestamp-rebind.v1"
    assert receipt["policy"] == "FUSE_SUCCESSOR_MTIME_CTIME_REATTESTATION"
    assert receipt["changed_fields"] == {"successor_mp4": ["mtime_ns", "ctime_ns"]}
    assert receipt["previous_bindings"]["successor_source"]["sha256"] is None
    assert receipt["current_bindings"]["successor_source"]["sha256"] is None
    successor_relative = f"{successor.parent.name}/{successor.name}"
    assert receipt["current_bindings"]["successor_mp4"]["sha256"] == finalized[
        successor_relative
    ]["target_sha256"]
    for field in ("size_bytes", "device", "inode", "mode"):
        assert receipt["previous_bindings"]["successor_mp4"][field] == receipt[
            "current_bindings"
        ]["successor_mp4"][field]
...
def test_connection_stub_fuse_timestamp_rebind_mp4_only_rejects_byte_drift(
    tmp_path: Path,
    monkeypatch,
) -> None:
    stub, successor, webhook_files, finalized = _connection_stub_fixture(tmp_path, monkeypatch)
    ...
    successor_mp4 = successor.with_suffix(".mp4")
    successor_mp4.write_bytes(b"x" * successor_mp4.stat().st_size)
    monkeypatch.setattr(adapter, "_mount_identity_for_path", _fuse_mount_identity)
...
@pytest.mark.parametrize("role", ["successor_source"])
