...
def test_post_semantic_entity_stage_never_reverts_name_to_draft_witness(tmp_path):
    """实案抽象：LLM/词表已把 draft 怪词修成专名后，后置
...
def test_unproven_foreign_cue_does_not_defer_to_partial_source_truth():
    ...
    pipeline.defer_unproven_foreign_introductions_to_late_authority(
        audit,
        # actual shape: the broad 毁神 window overlaps the cue but
...
def test_pinned_replay_ownership_requires_both_pins():
    ...
    for mutate in (
        ...
        lambda s: s["recovery_publication_authority"].update(title_mode="reviewer_manual_override"),
...
def test_final_review_provider_outage_writes_the_verbatim_cascade(monkeypatch):
    ...
    事故的取证成本全在这里：盘上只有 ``detail: "LlmCallError"``，
