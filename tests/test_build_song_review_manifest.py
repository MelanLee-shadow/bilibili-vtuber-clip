...
def _seal_talk_refresh_block(state: dict, *, recording_date: str) -> str:
    ...
    state["operator_processing_scope"] = {
        ...
        "grant_id": "synthetic-refresh-grant-001",
        "recording_date": recording_date,
        "reason": "synthetic review scope requires a no-upload rerun.",
        "candidate_ids": [talk_id],
        "user_authorization": {
            "quote": "keep this synthetic review run local",
...
def test_build_allows_completed_song_package_when_only_talk_refresh_is_blocked(
    ...
    """Synthetic Talk-refresh shape: Talk remains blocked; Song stays no-upload."""
    fx = _fixture(tmp_path)
