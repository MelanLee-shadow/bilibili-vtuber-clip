"""Pytest 密闭性守卫：单测永不真打 LLM 网络。

套件的承诺是"无凭据/无网络可跑、LLM 边界全部 mock"。这个承诺以前只在
无凭据机器上偶然成立：开发机 shell 若带着真实 CPA 凭据，mock 打在死
seam 上的测试会静默走真网络"变绿"——每次全量套件都在真烧配额，且测
试结果取决于远端模型。这里把真实 CPA 命令通道在 pytest 进程内机械封
死：任何测试打到它都显式失败，而不是花钱变绿。

需要 LLM 输出的测试请 monkeypatch 使用方模块自己的 llm-call builder
（例如 ``pipeline._build_final_review_llm_call``）；patch
``build_llm_call`` 只对"该模块自己调用自己这个名字"的 seam 有效。
