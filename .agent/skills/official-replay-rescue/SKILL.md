...
1. **找回放 BV**：space.bilibili.com/<UID>/lists → 系列·直播回放。
   record API（xlive/…/record/getList）常年为空且机房 IP 会 -352，别依赖。
2. **下载**（录制主机）：BBDown + SESSDATA cookies（模板与校验见 [docs/credentials.md](../../../docs/credentials.md) 第 6 条）
...
- **绝对时间锚点优先级：画面内时钟 > 官方标题的场次时间**。不少直播间 overlay
  ...
  注意 **overlay 时钟自身相对回放轴会漂**（实测 ~8 秒/小时）：两点外推的绝对
  误差按锚点如实写进 provenance（≤10s 量级可接受——段内时间轴与弹幕轴同源
  于回放轴，内部一致性是精确的）。
...
> **全新用户的主路径就是这一节**（没有任何录播姬幸存件时，上面的补洞流程都
> 用不上）。配套脚本：`scripts/build_session_from_replay.py`——切好段 mp4、
> 用画面时钟定好 t0 之后，三件套 sidecar + provenance 一条命令生成。
> **至少造两个连续段**：边界复核需要看到终点之后的下一个话题，单段源的段尾
> 候选会因 `BOUNDARY_SOURCE_WITNESS_RESERVE_INCOMPLETE` 交付不了。
