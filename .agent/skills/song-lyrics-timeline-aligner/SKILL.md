...
source/LRC evidence: AGY v5 must prove one continuous, predominantly sung live
Li Dousha performance with no other singer/harmony and no recorded/playback
vocal. The only non-sung exception is one short block of exact canonical
theatrical lyric lines performed by Li Dousha; the first/actual-final performed
rows and at least 80% (minimum seven) of performed rows must still be sung. A
complete live arrangement may omit one bounded repeated canonical block; code
must still prove at least 8 heard rows, 70% canonical coverage, 30 seconds,
head/middle/tail coverage, one global shift, a real live opening/ending, and a
post-song talk or instrumental-outro boundary. CAM++ independently samples seven
distinct rows explicitly labelled `SINGING_THIS_LYRIC`; at least 5/7 plus
head/middle/tail coverage must pass, and spoken rows can never supply a
checkpoint. Only their AND
result is `VERIFIED_LIDOUSHA_SINGING`. Missing, unavailable, uncertain,
guest/duet/harmony, other-singer, speech-over-music, offscreen/static replay, or
playback-only evidence fails closed.
...
2. Search for an external timed lyric source.
   ...
   - Search the clean song title first, including the original Japanese title/kana when available. Google or another public-web search is appropriate for manual discovery, but a search-result page is not an unattended timed-LRC API or proof source; save the direct lyric source instead. Do not make a garbled singing-ASR transcript the only query. In the unattended runner, `--lrc-provider auto` queries NetEase, LRCLIB, and Kugou, groups provider variants by normalized title+artist or identical full-LRC fingerprint, and prefers the canonical LRCLIB record when the identity is otherwise the same. Kugou candidates must exactly match their catalog title, artist, and duration, and its timed `[00:00] title - artist` card is metadata rather than a lyric. Japanese language or sparse/garbled singing ASR is not itself a terminal failure.
...
8. Auto-slice integration contract (implemented in `src/autoslice/full_session_candidate_selector.py` and `scripts/run_auto_review_shadow_pipeline.py`).
   ...
   - The audio fallback must inspect the current full proof window and canonical LRC in a sandboxed `agy` `Gemini 3.5 Flash (High)` run. Code, not the model, mints lyric-alignment proof: source/LRC/prompt/raw-output/run-manifest hashes must bind; every canonical row must be affirmatively classified as heard or not heard at confidence at least 0.8; heard starts must be strictly monotonic with at most 250 ms adjacent overlap; one global shift must explain every performed line within ±1500 ms; tempo drift needs separate stretch proof; and first line/chorus/repeated section/longest instrumental gap/tail plus the post-song transition must be checked. `FULL_STUDIO_SEQUENCE` requires every row; `COMPLETE_LIVE_ARRANGEMENT` may omit only one repeated block capped at 12 rows/30%, while retaining at least 8 rows/70%, 30 seconds, head/middle/tail, the real opening/ending, and a bound host-talk or instrumental-outro transition. The report retains the full canonical sequence but materializes subtitles only for heard/performed rows. A malformed, mismatching, incomplete, or fallback-model observation remains blocked.
   - The same AGY v5 observation is a hard same-subject live-performance veto. In addition to `mode=LIVE_STREAMER_SINGING`, confidence `>=0.85`, `continuous_live_song_performance=true`, `same_lidousha_live_performer_across_all_lyrics=true`, `background_recording_likelihood<=0.20`, and exactly three evidence timestamps that land in sung rows across lyric head/middle/tail, every heard canonical-LRC row must name Li Dousha as the same live lyric source and exclude another singer/harmony and recorded/playback vocal. At least seven and 80% of performed rows plus the first/actual-final performed rows must be `SINGING_THIS_LYRIC`. The only allowed exception is one contiguous `PERFORMING_THIS_LYRIC_SPOKEN` block of exact theatrical lyric lines: at most six rows, 12 seconds voiced, 20% of lyric-vocal duration, and 15 seconds first-to-last span. Ordinary speech/commentary/ad-libs remain `SPEAKING_NOT_SINGING` and block. `ORIGINAL_OR_BACKGROUND_PLAYBACK`, `OTHER_SINGER`, `STREAMER_TALKING_OVER_MUSIC`, `AMBIGUOUS`, guest/duet/harmony, offscreen/static replay, a missing row field, raw/report disagreement, or malformed evidence blocks even if LRC alignment is perfect and CAM++ sees Li Dousha speech.
   ...
   - The same job must also carry a separately generated `host-vocal-proof.v2`. First extract 4–8 seconds of post-song host speech and require its median against three hash-pinned Li Dousha enrollments to be `>=0.60`. Then select seven distinct, actually aligned `SINGING_THIS_LYRIC` cues of at least 2.5 seconds, skipping every spoken row; sample the central 2.5–4 seconds of each and require both the three-enrollment median `>=0.31` and the same-session host-anchor score `>=0.31`. At least 5/7 checkpoints and at least one in each of head/middle/tail must pass. This subclaim is honestly named `LIDOUSHA_VOCAL_PRESENT_ON_LYRIC_CHECKPOINTS`; it does not by itself prove that the matching voice is singing rather than talking over music.
   - The runner verifies source/alignment/profile/model/reference/session-anchor/checkpoint hash bindings, requires seven distinct decoded-PCM checkpoint hashes, binds and rechecks each checkpoint's `SINGING_THIS_LYRIC` assertions, and recomputes recorded score medians, thresholds, and bucket coverage. It also projects and compares the AGY v5 raw/report row-level singer/role and live-arrangement claims. It does **not** rerun CAM++ inference, so do not describe this as an independent ML reclassification or formal identity proof. Historical AGY v3 evidence remains readable only in the pinned 2026-07-09 incident-repair path; production proof generation and verification require v5/v2.
   ...
   - Once a source interval is identified as song-like, the quarantine expands to the same anchor-minus-45s / anchor-plus-45s full-proof retry range (capped to the segment), and all overlapping talk candidates are persistently removed before prioritization. A failed song proof cannot be laundered into a talk recut by taking only the intro/tail, changing candidate id, nesting a child window, or retrying through another lane.
