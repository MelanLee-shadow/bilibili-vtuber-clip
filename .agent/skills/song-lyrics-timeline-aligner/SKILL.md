...
source/LRC evidence: AGY v4 must prove one continuous, predominantly sung live
...
theatrical lyric lines performed by Li Dousha; first/final rows and at least 80%
(minimum seven) of rows must still be sung. CAM++ independently samples seven
...
8. Auto-slice integration contract (implemented in `src/autoslice/full_session_candidate_selector.py` and `scripts/run_auto_review_shadow_pipeline.py`).
   ...
   - The runner verifies source/alignment/profile/model/reference/session-anchor/checkpoint hash bindings, requires seven distinct decoded-PCM checkpoint hashes, binds and rechecks each checkpoint's `SINGING_THIS_LYRIC` assertions, and recomputes recorded score medians, thresholds, and bucket coverage. It also projects and compares the AGY v4 raw/report row-level singer/role claims. It does **not** rerun CAM++ inference, so do not describe this as an independent ML reclassification or formal identity proof. Historical AGY v3 evidence remains readable only in the pinned 2026-07-09 incident-repair path; production proof generation and verification require v4/v2.
