...
source/LRC evidence: AGY v3 must prove on every heard lyric row that Li Dousha
is the one singing that lyric from the same live vocal source, with no other
singer/harmony and no recorded/playback vocal; CAM++ must independently find
Li Dousha vocal identity on distinct selected lyric checkpoints. Only their AND
...
8. Auto-slice integration contract (implemented in `src/autoslice/full_session_candidate_selector.py` and `scripts/run_auto_review_shadow_pipeline.py`).
   ...
   - The same AGY v3 observation is a hard same-subject live-performance veto. In addition to `mode=LIVE_STREAMER_SINGING`, confidence `>=0.85`, `continuous_singing=true`, `background_recording_likelihood<=0.20`, and exactly three specific evidence timestamps covering lyric head/middle/tail, every heard canonical-LRC row must say `lyric_vocal_subject=LIDOUSHA`, `lidousha_role=SINGING_THIS_LYRIC`, and `same_live_vocal_source_as_lidousha=true`. Every row and the recomputed aggregate must exclude another singer/harmony and recorded/playback vocal. `ORIGINAL_OR_BACKGROUND_PLAYBACK`, `OTHER_SINGER`, `STREAMER_TALKING_OVER_MUSIC`, `AMBIGUOUS`, guest/duet/harmony, offscreen/static replay, a missing row field, raw/report disagreement, or malformed evidence blocks even if all LRC lines align and CAM++ sees Li Dousha speech.
   ...
   - The runner verifies source/alignment/profile/model/reference/session-anchor/checkpoint hash bindings, requires seven distinct decoded-PCM checkpoint hashes, and recomputes recorded score medians, thresholds, and bucket coverage. It also projects and compares the AGY v3 raw/report row-level singer/role claims. It does **not** rerun CAM++ inference, so do not describe this as an independent ML reclassification or formal identity proof.
