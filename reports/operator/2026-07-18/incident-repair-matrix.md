...
|---|---:|---|---|
| 公会片 0:25 | 20.712s 附近 | `kmx已经成为了李豆沙的帕鲁` cue 覆盖完整语音 | 已按 BCUT/v2 时间重烧并验收 |
| 打灰片 0:17 | 09.970–12.250 | `不行不行，并非不行` | 已重烧并验收 |
| 打灰片 0:54 / 1:01 | 46.280–47.396、52.380s 后 | `与你打灰到生命尽头` 及后续语音不被 VAD 短裁 | 已按 BCUT/v2 时间重烧并验收 |
| 妈感片 约 1:02 | 57.530–60.450 | 删除 watched-video 日语 `裏表すごいし` | 已 drop、重烧并验收 |
| 妈感片 2:09 | 125.090–127.650 | `soyo就是妈`，逐字复制已确认读出的弹幕 | 已重烧并验收 |
| 妈感片 3:03 | 177.190–179.070 | `哦，sumimi的。` | 已重烧并验收；`sumimi` 以 MyGO 剧情中出现的独立 `unit` 接入 |
| 抽卡片 0:34 | 28.040–29.780 | `李豆沙，说个台词` | 已重烧并验收 |
| 抽卡片 1:02–1:16 | cue 24/25/27/30 | 所有经音频确认的自指槽恢复 `李豆沙` | 已重烧并验收；平等 resolver 回归通过 |
| 抽卡片 3:05 / 3:13 | 181.670s、189.030s 后 | cue 使用 BCUT 正常时长，覆盖语音 | 已按 BCUT/v2 时间重烧并验收 |
| 抽卡片 3:18 | 191.890–193.542 | `李豆沙一直是为爱做1` | 已重烧并验收 |
| 抽卡片 3:22 | 195.450–197.650 | `李豆沙一直是……` | 已重烧并验收 |
| 抽卡片 3:24 | 199.090–200.090 | `侄女` | 已重烧并验收 |
...
- topic graph 的 35 项测试通过；`sumimi` 是 `unit`，通过
  ...
  直接来源已补为 MyGO 官方角色页和官方唱片页。
- 三份人工 override 的 source witness 和 decision witness SHA-256 均重新计算并验证，
  逐 cue 应用成功。
- 四个最终 MP4 均全解码通过，均含 4288 ms 片头，最终 SRT 与修复后的
  BCUT/v2 时间逐 cue 一致（妈感片仅少了明确 drop 的 watched-video 日语 cue）。
- 运行机与本地 canonical mirror 的最终 MP4 SHA-256 一致：

| 片 | 最终 MP4 SHA-256 |
|---|---|
| 公会片 | `3237bf394442de2f26005d730cab012f7bd78713edd4cf7a0940e417003126f0` |
| 打灰片 | `5c371614765bc6c9f8d206052b547fa70702ac05fe322e99cbfa2733ca329ce4` |
| 妈感片 | `82184bbc18825bffeb3602fa09743189c688fa50bd2e79bf935736bbc89a00d6` |
| 抽卡片 | `d2784264f99002763d2b4505c5b71cfe62d31e42dae879274200ddd9a4d84b4e` |
...
1. 四片 no-upload 修复已达到本次验收门；当前审片入口见同目录
   `current-review-index.json`。旧 `.text-finalization.json` /
   `.subtitle-regression.json` 只作历史证据，不再是当前成品 authority。
2. 未获 Ivan 明确授权，本任务不得执行 B 站换源或任何上传。
3. same-BVID 的长期 P0 是把内容寻址版本和原子 `current` pointer 内建到常规生产流程，
   不能长期依赖一次性 flat mirror 刷新。
