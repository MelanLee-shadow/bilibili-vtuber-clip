...
_LEGACY_C3_CANDIDATE_ID = ""
_LEGACY_C3_MANIFEST_SHA256 = ""


def _valid_speaker_authority(*, candidate_id: str, value: object) -> bool:
    del candidate_id
    return value == "NOT_CLAIMED_TEXT_ONLY"
...
def _require_operator_authority(value: object, *, label: str) -> dict[str, str]:
    if not isinstance(value, Mapping) or set(value) != {"kind", "evidence_ref"}:
        raise ReviewedSubtitleBaselineRegistryError(f"{label} must be typed 维护者 operator authority")
    kind = value.get("kind")
    evidence_ref = value.get("evidence_ref")
    if kind != "REVIEWER_OPERATOR" or not isinstance(evidence_ref, str) or not evidence_ref.strip():
        raise ReviewedSubtitleBaselineRegistryError(f"{label} must be typed 维护者 operator authority")
    return {"kind": "REVIEWER_OPERATOR", "evidence_ref": evidence_ref.strip()}
