...
_QUESTION = (
    ...
    "‘这个正在说话的人说了什么’：同片说话人的真实神态加具体提问、回应或引语，本身就能"
    "形成故事关联；被台词提及的人物、物件或动作不必同时出现在静帧中。字幕/故事事实检查"
    "负责台词是否有来源，本图像门不能仅凭没有画出动作或对象，就反推台词虚构、图文无关"
    "或‘通用头像硬凑故事’。身份正确或笑容本身仍不足以通过：已渲染文字必须给出可理解的"
    "具体事件、回应或悬念，且可见人物状态与之相容。若只有人物名/空泛口号、主体缺失，或"
    "具体可见内容与文字矛盾，仍须判 false，并指出这项实际缺陷而不是缺席的台词动作。通常脸或上半身"
    "应形成第一视觉焦点；仅仅能认出她不算通过。专门留给已渲染标题的干净文字区是合理"
    "留白；刻意设计的图形海报边距、边框和红/绿配色点缀（包括纯色色块）不自动算无意义"
    "装饰。但标题以外的大片死空白、真正无意义的纯色红条/色块、装饰噪声或与故事无关的"
    "强元素仍须拦截；只有装饰实际比人物/故事内容更强地成为视觉焦点、遮挡主体，或让"
    "缩略图难以理解时，才将meaningless_dominant_decoration判为true。陌生观众只看右图时应"
    "立即知道看谁、看到一个明确点击钩子。thumbnail_has_clear_click_hook 按右图画面与"
    "已渲染文字共同判断，具体的人物状态、互动或场景与标题可以共同呈现事件或悬念；"
    "不要求去掉文字后图像仍独立讲完故事，也不要求主播实际演出她描述或回应的动作。"
    "若图文合起来仍只认得人物、没有具体事件或悬念，或图文存在可指出的事实矛盾，仍须判 false。"
    "不确定就 FAIL。只输出 JSON："
