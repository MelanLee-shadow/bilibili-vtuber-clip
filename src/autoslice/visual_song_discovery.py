...
VISUAL_SONG_AGY_MODEL_ENV = "AUTOSLICE_VISUAL_SONG_AGY_MODEL"
...
class VisualSongConfig:
    ...
    sheet_rows: int = 3
    model: str = field(
        default_factory=lambda: os.environ.get(VISUAL_SONG_AGY_MODEL_ENV, DEFAULT_MODEL)
    )
...
def _summarize_provider_failures(
    *,
    ...
    def safe_row(row: Mapping[str, object]) -> dict[str, object]:
        ...
        for field_name in ("key_ordinal", "attempt_round"):
            value = row.get(field_name)
            if isinstance(value, int) and not isinstance(value, bool):
                safe[field_name] = max(0, min(value, 99))
        return safe
