...
def make_accepted_receipt(root: Path, proposal_path: Path, reviewed_at: str, decision_basis: str) -> dict[str, Any]:
    ...
    return {
        ...
        "required_root_checks": ROOT_CHECKS,
        "non_authorizations": [
            "does not replace 维护者 line947 authorization",
...
def validate_accepted_receipt(root: Path, proposal_path: Path, receipt: Mapping[str, Any], *, allow_audit_root_relocation: bool = False) -> None:
    ...
    if receipt["non_authorizations"] != ["does not replace 维护者 line947 authorization", "does not replace CPA title-cover QC", "does not authorize AUTO_UPLOAD or any upload"]:
...
def _replay_current_audit(root: Path, *, allow_root_relocation: bool = False) -> None:
    ...
    _regular(audit_path)
    auditor = Path(__file__).resolve().parents[2] / "scripts" / "audit_review_package.py"
    _regular(auditor)
    run = subprocess.run(
