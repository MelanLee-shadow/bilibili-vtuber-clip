...
ordering-and-rubric.txt`，Ivan 已批准）解决的是这个次序矛盾：`prioritize()` 在
说话人路由**之前**跑，召回侧纯文本零说话人标注，于是 `lidousha_centrality`
在物理上判不了主角——2026-08-10 Ivan 盲审四条 tier-1 的真值证明它与真值**反相关**
（`docs/reviews/2026-08-10-ivan-blind-review-tier1-ground-truth.md`）。
...
Ivan 2026-08-10 对 Pro 这条纠正的逐字裁定是「可以」。所以本模块**对进入争席的
...
停泊转人工（Ivan「说话人存疑都要直接给人工审阅」），**不许猜成 SOLO 放行**。
...
- 归属状态映射到 ``selection_metric_v2`` 既有的 ``attribution_status`` 词汇，
  ...
  ``VERIFIED_HOST_DOMINANT`` 是**事实错误**（Ivan 说的正是"主要发言人不是李豆沙"），
...
只是在**争席排序**时不看它——它怎么从 v1 评分卡退出是下一步，要 Ivan 单独裁。
"""
...
# --- 争席次序（Ivan 2026-08-10 逐字：「N 可以选 10 个，不够了再补上」） ---
...
def map_attribution_status(occupancy: Mapping[str, object]) -> str:
    ...
    是**事实错误**（那正是 Ivan 盲审对两条候选的原话）；而把它叫 ``UNVERIFIED``
    会把一条**归属已经确定**的候选送去人工，既淹没人工队列（Pro §5：能靠其他
    独立条件安全淘汰的候选不必送审），也违反 Ivan「最好是能够自然给出低分，
...
def park_unverified_candidate(record: dict, occupancy: Mapping[str, object]) -> dict | None:
    ...
    return park_for_manual_review(
        ...
        migrated_from={
            ...
            "estimator_version": ESTIMATOR_VERSION,
            "threshold_version": THRESHOLD_VERSION,
            "authority": "IVAN_2026-08-10_SPEAKER_UNCERTAIN_GOES_TO_HUMAN_REVIEW",
