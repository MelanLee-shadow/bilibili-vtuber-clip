...
Extracted from scripts/session_autoslice.py (屎山治理第三刀).
...
def _validate_repaired_cover_generation(
    *, cover: Path, title: str, candidate_id: str
    ...
    if document.get("workflow") != "regenerate_channel_cover":
...
def _enrich_repaired_cover_generation(
    *,
    ...
    ``regenerate_channel_cover`` owns the paid image result, but it does not
...
def bind_manual_package_cover(*, package_dir: Path, cover: Path) -> dict:
    ...
    `regenerate_channel_cover` 手补的 cover.png 曾经无处回写——包内
    ...
    import scripts.session_autoslice  # noqa: F401  (RunnerProxy 解析面)
...
def cover_repair_needed(date: str, rec: dict) -> bool:
    ...
    # what the ADVISORY semantic judge said (维护者).  Blocked/failed
