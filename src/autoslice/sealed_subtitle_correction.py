"""Candidate-sealed replay authority for the 2026-08-17 女友感 repair.
    ...
    for position, (source, row) in enumerate(zip(source_rows, rows), start=1):
        ...
        if position in replacements:
            ...
            if set(row) != set(common) | {
                ...
                or row.get("decision_authority") != "IVAN_OPERATOR"
                or row.get("evidence_ref") != "Ivan 2026-08-19 Claude JSONL line 947 exact cue correction."
            ):
            ...
            if set(row) != set(common) | {"release_text", "disposition", "assertion"} or (
                ...
                != "Ivan line947 reports this text; assertion only, not a full-ownership decision."
            ):
