...
def validate_correction_authority(
    ...
    *,
    ...
    ivan = dict(_require_mapping(authority.get("ivan_authority"), label="ivan_authority"))
    if set(ivan) != {"story_truth_quote", "correction_quote", "publication_quote"}:
        raise SourceFactRescoreProvenanceError("ivan_authority field set is invalid")
    for key in sorted(ivan):
        _require_nonempty_string(ivan.get(key), label=f"ivan_authority.{key}")
