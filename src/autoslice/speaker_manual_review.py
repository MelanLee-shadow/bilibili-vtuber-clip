...
Ivan 2026-08-10 裁定：「说话人证据不足应该转人工审阅，不是判死」，理由是
...
- 复用既有状态名 ``speaker_review_required`` / ``speaker_evidence_insufficient``。
  ...
  **从来不在** ``DELIVERED_TALK_STATUSES``（``{"ok","review_ready","quarantine"}``）里
  ...
  （``build_lidousha_daily_review_manifest.py`` 要求 ``status == "review_ready"``）。
...
- 出版登记（``publication_registry.py`` 的 ``hold_pending_review``）是**上传**
  ...
  运行时不得代 Ivan 往那份登记里写行（7667d9a 血泪：hold 要写仓内资产）。
...
的指纹里：说话人模块代码波，或 Ivan 落一份 ``candidate_speaker_override_path``
    ...
    for record in state.get("picks") or []:
        ...
        park_for_manual_review(
            ...
            migrated_from={
                ...
                "authority": "IVAN_2026-08-10_SPEAKER_EVIDENCE_GOES_TO_HUMAN_REVIEW",
            },
        )
        restored += 1
    return restored
...
def render_report_section(
    ...
    lines = [
        ...
        "",
        "> 说话人分离是新开的功能，证据不足不判死（Ivan 2026-08-10）。这些候选停在"
