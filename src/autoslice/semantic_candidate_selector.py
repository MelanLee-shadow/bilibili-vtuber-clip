...
def _slice_selection_metric() -> str:
    """维护者's curated slice-selection metric (single authority asset).
...
"""
    ...
    metric_block = (
        f"\n选题优先级 metric(维护者 逐条校准过的权威,选题和排序都必须对照它;历史真例/反例都在里面):\n{metric}\n"
        if metric
        else ""
    )
    scope_block = f"\n召回范围说明:{scope_note}\n" if scope_note else ""
    ...
    *,
    ...
    for confidence_value, spec in parsed:
        ...
        if isinstance(scorecard, dict):
            # 狍哥案修复（design §3.5）：start_cue/end_cue 此前只
...
def plan_semantic_recall_shards(
    ...
    *,
    ...
    One huge two-hour prompt under-recalled the stream: it returned
...
def rescore_candidate_scorecard(
    *,
    ...
    other candidate (design §4, 狍哥案修复).
