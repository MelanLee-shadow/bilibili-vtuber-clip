...
def is_fastlane_formal_manifest(manifest: Mapping[str, object]) -> bool:
    ...
    return is_fastlane_c1_formal_manifest(manifest) or is_fastlane_c2_formal_manifest(manifest) or is_fastlane_c2_release_manifest(manifest)
...
def audit_fastlane_formal_package(root: Path, manifest: Mapping[str, object]) -> list[C1AuditIssue]:
    ...
    from .fastlane_c2_release_bridge import audit_fastlane_c2_release_package, is_fastlane_c2_release_manifest
    if is_fastlane_c2_release_manifest(manifest):
        return [C1AuditIssue(item["code"], root, item.get("detail", "")) for item in audit_fastlane_c2_release_package(root)]
