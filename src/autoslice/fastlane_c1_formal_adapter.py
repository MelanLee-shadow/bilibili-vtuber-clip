...
def _validate_formal_authority(value: Mapping[str, object], *, repo_root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or ruling.get("reviewer_rereview_required") is not False
    ...
    if (
        ...
        != {
            ...
            "insertion": {
                ...
                "basis": "维护者 ruling location: about 01:37 on the public burn; C1's source SRT is main-delivery-local and Z1 contributes 5749ms, so this is 01:31.100–01:31.900 on the source grid.",
            },
        }
    ):
    ...
    expected_names = {
        ...
        "ruling_document": "ruling/2026-08-19-维护者-review-batch-rulings.md",
...
def _public_identity_binding(snapshot_path: Path, authority: Mapping[str, object]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        != {
            ...
            "preserve_all_live_metadata_unless_reviewer_changed": True,
    ...
    return (
        {
            ...
            "reviewer_rereview_required": False,
        },
        raw_document,
    )
...
def _validate_technical_template(root: Path, authority: Mapping[str, object], manifest_path: Path) -> None:
    ...
    expected = {
        ...
        "required_checks": [
            ...
            "inspect the six 维护者-named subtitle points on the canonical burned final",
