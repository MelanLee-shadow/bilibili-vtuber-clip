...
Ivan 2026-08-10 逐字裁定：「我认为这些地方不应该分散，反而是防止代码屎山的
...
The second standing ruling this module encodes is Ivan's repeated instruction
...
* 2026-07-19 (entity/LRC lanes): a pure-quota free-chain round may be repeated
  back-to-back inside one run until the >=3 strike policy is satisfiable.
* 2026-07-20 (foreign-span lane): a fully-429 free chain is deterministic
...
def run_gemini_key_ladder(
    *,
    ...
    ``max_free_rounds``
        ...
        every failure in the round was quota-class (2026-07-19).  Defaults to
    ...
    #     whether another free round may run (2026-07-19).
    #   flags      -> the broad 429/resource-exhausted sniff that decides the
    #     same-round paid fastpath (2026-07-20).
    categories: list[str] = []
