...
治 C7（2026-08-10 截图优先）：通用 cover-only 修复是**重绘独占**的
（`cover_repair.py` 三处硬编码 `selected_treatment="cpa_redraw"`，
`scripts/regenerate_lidousha_cover.py` 自述 "real CPA image edit only"），于是任何
...
def recoverable_screenshot_route(rec: object) -> tuple[str, Path] | None:
    ...
    ``<cover>.cover_generation.json`` 只有 `scripts/regenerate_lidousha_cover.py`
