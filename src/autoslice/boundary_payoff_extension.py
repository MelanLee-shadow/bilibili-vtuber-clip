...
## 为什么有这个模块（2026-08-10，Ivan 盲审 8/7 tier-1 的真值）

Ivan 不看分数直接看原片，判定 `auto_223750_578_654`（评分器排最低的 69.5）是四条里
...
def _adopt_run_end_ms(
    ...
    (Ivan 判据 1：落点恒在人声边界), or ``end_ms`` when nothing is adoptable.
    """
