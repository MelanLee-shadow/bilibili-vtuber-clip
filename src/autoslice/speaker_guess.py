...
``guest_score``/``margin``——正好是 Ivan 定位"该改哪几句"要的东西。
...
**梯子有底**（Ivan「无论如何」的落地）。三级，逐级降级、逐级可区分：
...
``IDENTITY_INDETERMINATE_UNIFORM_HOST``
    ...
    这一级 Ivan 明说修正成本高，所以它**只是兜底**，receipt 的 ``rung`` 与报表
...
#: 每条 cue 的证据缺口码。Ivan 要的是"改哪几句"，所以缺口逐 cue 记，不是整片一个标记。
...
def nominate_host_anchors(
    ...
    *,
    ...
    ``rows`` 逐条写明该 cue 的 seed 分与它**没有**清过的阈值，这样 Ivan 一眼
...
def build_guess_receipt(
    ...
    *,
    ...
    receipt: dict[str, object] = {
        ...
        # 被提名成主播的那簇其实是客人。Ivan 改一次极性就能翻回来，但他得先知道
...
def delivered_artifact_paths(result: Mapping[str, object]) -> dict[str, object]:
    ...
    两个作用：Ivan 从报表直接拿到"打开哪个文件看"；``free`` 的容量清理在删
