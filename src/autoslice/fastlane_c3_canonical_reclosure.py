...
from src.autoslice.fastlane_c3_terminal_source_fact_preservation import (
    CANDIDATE_ID,
    build_review,
    load_authority as load_source_fact_authority,
)
...
def reclose_c3_canonical_package(*, v3_root: Path, destination: Path, repo_root: Path) -> C3CanonicalReclosureResult:
    ...
    source_auth = load_source_fact_authority(repo_root=repo_root)
    ...
    try:
        ...
        speaker_evidence = rebuild_speaker_evidence(record, _source_cues(subtitle_path), speaker_srt_bytes=speaker_srt_path.read_bytes(), speaker_manifest_bytes=speaker_manifest_path.read_bytes()).speaker_evidence
        ...
        if clip_context_prompt_text(clip_doc) != str(story.get("clip_context_prompt") or ""):
            raise C3CanonicalReclosureError("C3_CLIP_CONTEXT_AUTHORITY_MISSING")
        source_fact = build_review(repo_root=repo_root, title=str(publish["title"]), selection_hook=str(story.get("selection_hook") or ""), selection_scorecard=story.get("selection_scorecard"), clip_context_prompt=str(story.get("clip_context_prompt") or ""), subtitle_text=subtitle_path.read_text(encoding="utf-8"), speaker_srt=speaker_srt_path.read_bytes(), speaker_finalization=speaker_manifest_path.read_bytes(), speaker_evidence=speaker_evidence)
        ...
        boundary = derive_current_boundary_review(stale_review=stale_review, cues=cues, owner_verification=record["boundary_audit"]["required_boundary_owner_verification"], coverage_verification=record["boundary_audit"]["delivery_coverage_verification"], authority=boundary_auth)
        before_docs = {"record": record, "publish": publish, "chat": chat}
