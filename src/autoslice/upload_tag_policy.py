...
UPLOAD_TAG_POLICY_SCHEMA = "vtuber-slice.upload-tag-policy.v1"
...
def load_upload_tag_policy(path: Path) -> UploadTagPolicy:
    ...
    raw_rules = root.get("term_rules")
    if not isinstance(raw_rules, list):
        raise UploadTagPolicyError("term_rules must be a list")
    rules: list[TermRule] = []
    for index, raw_rule in enumerate(raw_rules):
        label = f"term_rules[{index}]"
        ...
        patterns = _string_list(rule.get("patterns"), label=f"{label}.patterns")
        for pattern in patterns:
            try:
                re.compile(pattern)
            except re.error as exc:
                raise UploadTagPolicyError(
                    f"{label}.patterns contains invalid regex {pattern!r}: {exc}"
                ) from exc
        rules.append(
            TermRule(
                name=_string(rule.get("name"), label=f"{label}.name"),
                patterns=patterns,
                tags=_string_list(rule.get("tags"), label=f"{label}.tags"),
                min_hits=_positive_int(rule.get("min_hits"), label=f"{label}.min_hits"),
                note=_string(rule.get("note"), label=f"{label}.note", allow_empty=True),
            )
        )
    names = [rule.name for rule in rules]
    if len(names) != len(set(names)):
        raise UploadTagPolicyError("term_rules must not repeat rule names")
