    ...
    *, story_hook: str, cover_text: str, llm_factory=build_llm_call,
    ...
    try:
        llm = llm_factory(LlmConfig(
            transport="command",
            command_template="bash scripts/llm_via_cpa.sh {prompt_file} {completion_file} 'gpt-6-sol' medium",
...
def _production_stage_cover(
    *, authority: Mapping[str, object], root: Path, runtime: Mapping[str, bytes], approved_punch: tuple[str, ...],
    ...
    from src.autoslice.publish_staging import _stage_ai_cover
    ...
    result = _stage_ai_cover(
        ...
        source_composition_verifier=verify_source_composition,
    )
