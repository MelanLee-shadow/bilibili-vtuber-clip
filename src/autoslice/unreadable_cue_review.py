...
Ivan 2026-08-10 逐字：「……应该直接报需要审查，并且在权宜上传时也不能上传，
可以把这段字幕删掉然后出成品等待审阅，而不是拦住。」

三条硬要求分别落在哪：

1. **报「需要人工审查」** —— 本模块的停泊回执 + 报表专章
   （``render_report_section``），Ivan 一眼看到「这里少了一句话，为什么少」。
2. **权宜/快速上传通道也不能上传** —— 靠状态承载，不靠回执。
   ``UNREADABLE_CUE_REVIEW_STATUS`` **不在**
   ``free_session_autoslice.DELIVERED_TALK_STATUSES``（``{"ok","review_ready",
...
3. **出成品等待审阅、不拦死** —— 删除发生在终审自愈循环里，release gate 照常
   PASS，媒体照常烧录；成品路径写进停泊回执，Ivan 打得开。
...
已提交资产上的人工裁定，运行时**不得**代 Ivan 写行（7667d9a 血泪，而且
...
def render_report_section(
    ...
    """Ivan 一眼能看到「这条少了一句话，是因为它物理上听不清」的那张表。"""
    ...
    lines = [
        ...
        "超出普通话音节率上限），既不是「机器没跑」也不是「内容有问题」。按 Ivan "
