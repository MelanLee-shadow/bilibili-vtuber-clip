...
模型选型（2026-07-25 基准，Ivan 规则=准确度优先、同准确度才降级）：
gpt-5.6-sol 默认——3 轮重复基准关键专名逐字命中 3/3（terra 仅 1/3，两轮
错字豆丁/豆豆；视觉模型存在轮间抖动，单轮基准不可靠）。gpt-5.5 漏关键
ID；gpt-5.6-luna 无中生有幻觉（编造不存在的字幕），证据场景禁用。
