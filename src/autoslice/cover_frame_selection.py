"""Expressive cover-frame selection（维护者：截图直出封面试点）。
...
≥2.5s）与全片动作热区 bbox（用于放大脸部占比的裁切锚点，维护者 批准
...
# speech, emotion, and sharpness (1493 picked a foggy game
...
def select_expressive_cover_frame(
    ...
    *,
    ...
    # 片头→正片过渡自适应（实锤：z1-budui 片头 5749ms > 固定 skip
...
def extract_zoomed_cover_frame(
    ...
    *,
    ...
    维护者 批准加大脸部占比：默认 1.32x，裁切窗锚定动作质心水平位置、
    顶边贴 avatar 头顶（顺带裁掉底部烧录字幕带）。无锚点时中央裁切。
    ``window_bbox_frac``（游戏场小窗回归）：显式主播小窗 bbox，
