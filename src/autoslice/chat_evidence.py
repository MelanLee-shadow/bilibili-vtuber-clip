...
class ReferentGroup:
    ...
    # 方向性 fail-closed：列在此处的 canonical 是组内
...
def build_human_text_entity_verifier(
    ...
    *,
    ...
    """Build a deferred verifier from one hash-bound 维护者 text decision asset.
    ...
    def verifier(request: Mapping[str, Any]) -> Mapping[str, Any] | None:
        ...
        return {
            ...
            **binding,
            "authority": str(row.get("authority") or "维护者 direct correction"),
        }
...
def reconcile_pending_text_overrides(
    ...
    *,
    ...
    def rebind_deferred_verdict(
        ...
        """Move an unchanged 维护者 entity verdict onto a newly frozen ASR variant.

        A generative text pass may drift between attempts even though the
        evidence id and 维护者's entity decision do not.  Rebinding is allowed
