...
def _profile_fontsdir(media_path: Path | None = None) -> Path | None:
...
def _cover_art_direction_prompt(
    *,
    ...
    return (
        ...
        f"画面/显示器/UI，转译成字面的动物、龙、模型机或屏幕道具**。{CHANNEL_PROFILE.cover_identity.scene_prop_meme_note_zh}"
...
def _cover_prompt(
    *,
    ...
    """Text-free CPA background prompt, ART-DIRECTED per clip (维护者).
    ...
    ``emote`` (维护者): when the art direction carries a strong-reason
    ...
    if art_direction is None:
        art_direction = _cover_art_direction(candidate_id="", title=title, cover_text=cover_text)
...
def _cover_screenshot_polish_prompt() -> str:
    ...
    return (
        ...
        "Do not add large empty regions, solid-color bars, decorative blocks, props, or visual noise; every retained "
        "graphic element must support the visible subject and story. "
...
# of 16 with HTTP 400 model_price_error — 1080 isn't one (first hit,
...
def _split_wide_atom(atom: str, max_em: float, protect: str = "") -> list[str]:
    ...
    维护者 的排版铁律是 词/hook/专名 不可拆、其余任意断行；但 LLM 偶尔把整个
...
def _fit_cover_lines(cover_text, *, hook_word, base_fill, hook_rgb, zone, font_path, max_lines=3, max_size=300, forced_lines=(), word_atoms=(), locked_lines=()):
    ...
    ``word_atoms`` (维护者): the art direction's validated word
...
def _talk_locked_split(
    ...
    auto_192000_909_1014（白色奶龙）就是没有任何权威时由操作者手写文案、
...
def _overlay_cover_title(
    ...
    *,
    ...
    reference covers (the dark card/glow modes exist but 维护者 rejected the card
    ...
    if art_direction is None:
        art_direction = _cover_art_direction(candidate_id="", title=cover_text, cover_text=cover_text)
    ...
    if punch_lines:
        ...
        # talk 的 mode=full 自动回退在 收窄：只有当 cover_text 确实装不下
...
def _find_cover_font() -> Path:
    return resolve_primary_cover_font(
        channel_profile=CHANNEL_PROFILE, root=ROOT, fontsdir=_profile_fontsdir(None)
    )
