"""Strict v7 recovery lineage for one selected final-review rejection."""
...
from src.autoslice.final_review_provider_budget_retry import (
    ...
    valid_retry_evidence,
)
from src.autoslice.published_topic_recovery_lineage import RECOVERY_REBOUND_FIELDS
from src.autoslice.runner_proxy import RunnerProxy


RECEIPT_SCHEMA = "selected-final-review-recovery-receipt.v1"
RECOVERY_RECEIPT_FIELD = "selected_final_review_recovery"
READY_TO_REQUEUE = "READY_TO_REQUEUE"
OUTSTANDING = "OUTSTANDING"
CONVERGED = "CONVERGED"
BLOCKED = "BLOCKED"

_ACTION = "REQUEUED_BY_OPERATOR_FINAL_REVIEW_GRANT"
...
QUEUE_TO_PICK_TRANSITION = "QUEUE_TO_PICK"
PICK_TO_QUEUE_TRANSITION = "PICK_TO_QUEUE"
_ACTIVE_TALK_COLLECTIONS = (
    "pending_talk", "talk_backlog", "picks", "talk_below_confidence_threshold"
)
_QUEUED_TALK_COLLECTIONS = ("pending_talk", "talk_backlog")
_SONG_COLLECTIONS = (
    "pending_song", "song_backlog", "song_selection_backlog", "songs", "song_superseded_attempts"
)
_SUCCESS_TERMINAL_STATUSES = frozenset({"ok", "review_ready", "quarantine", "published"})
_KNOWN_FAILURE_STATUSES = frozenset({"failed", "candidate_rejected", "boundary_unrepairable", "speaker_review_required", "speaker_evidence_insufficient"})
_INFRA_FAILURE_STAGES = {
    "runtime_prerequisite": frozenset({"speaker_preflight", "source_media_binding"}),
    "provider_transient": frozenset({"boundary_semantic_review", "external_provider",
        "final_review_adjudication", "final_review_boundary_semantic",
        "final_review_correction_discovery", "final_review_discovery",
        "foreign_source_audio_witness", "source_fact_review"}),
}
_DETERMINISTIC_FAILURE_STAGES = {
    "candidate_gate": frozenset({"candidate_admission"}),
    "content_boundary": frozenset({"boundary_resolution", "boundary_semantic_review", "final_review_boundary_semantic"}),
    "content_duration": frozenset({"boundary_resolution"}),
    "final_review_contract": frozenset({"final_review_contract"}),
    "pipeline_contract": frozenset({"boundary_retry_owner_contract"}),
    "source_media": frozenset({"source_media_binding"}),
    "speaker_evidence": frozenset({"speaker_finalization"}),
    "story_contract": frozenset({"source_fact_repair", "subtitle_entity_consistency", "title_fact_consistency"}),
    "subtitle_authority": frozenset({"chat_authority_final_artifact", "chat_authority_finalization", "final_review_findings", "foreign_source_transcription", "redelivery_subtitle_baseline", "redelivery_terminal_projection"}),
}


@dataclass(frozen=True, slots=True)
class SelectedFinalReviewRecoveryInspection:
    outcome: str
    reason_code: str
...
def is_selected_final_review_rejection(row: Mapping[str, object]) -> bool:
    """Recognize only the exact selected 1576 final-review rejection."""
    ...
    return bool(
        ...
        and row.get("failure_stage") == "final_review_findings"
...
def build_selected_final_review_recovery_receipt(
    *,
    ...
    """Seal the exact old rejection and its first resulting queue row."""
    ...
    if not (
        ...
        and is_selected_final_review_rejection(old_row)
        and _is_queue_row(queued_row, candidate_id)
        and RECOVERY_RECEIPT_FIELD not in queued_row
    ):
        raise ValueError("selected final-review recovery receipt input is invalid")
    ...
    if not (
        ...
        raise ValueError("selected final-review recovery fingerprints are invalid")
    try:
        canonical_old = _canonical_copy(dict(old_row))
        queued_body = _canonical_copy(_row_body(queued_row))
    except (TypeError, ValueError) as exc:
        raise ValueError("selected final-review recovery rows are not canonical JSON") from exc
    if not isinstance(canonical_old, dict) or not isinstance(queued_body, dict):
        raise ValueError("selected final-review recovery rows are invalid")
...
def _receipt_is_self_valid(
    ...
    *,
    ...
    if not isinstance(receipt, Mapping) or set(receipt) != _RECEIPT_FIELDS:
        return False
    body = {str(key): value for key, value in receipt.items() if key != "receipt_sha256"}
    ...
    try:
        old_sha = _sha256_json(old_row)
        initial_sha = _sha256_json(initial_queue)
        current_sha = _sha256_json(current_row)
    ...
    if not (
        ...
        and receipt.get("old_row_sha256") == old_sha
        and receipt.get("initial_queue_row_sha256") == initial_sha
        and receipt.get("current_row_sha256") == current_sha
    ...
    for index, transition in enumerate(transitions):
        expected_kind = (
            _INITIAL_TRANSITION
            if index == 0
            else QUEUE_TO_PICK_TRANSITION
            if index % 2 == 1
            else PICK_TO_QUEUE_TRANSITION
        )
        if not (
    ...
    head_valid = (
        ...
        if head_kind == QUEUE_TO_PICK_TRANSITION
        else _is_queue_row(current_row, candidate_id)
    )
    return bool(
        transitions[0].get("to_row_sha256") == initial_sha
        and previous == current_sha
        and head_valid
    )


def validate_selected_final_review_recovery_receipt(
    receipt: object,
    *,
    queued_row: Mapping[str, object],
    candidate_id: str,
    ...
    """Validate the receipt against its current persisted queue row."""
    ...
    return bool(
        ...
        and transitions[-1].get("kind") != QUEUE_TO_PICK_TRANSITION
        and _changed_fields(current_row, actual).issubset(_QUEUE_REBOUND_FIELDS)
    )


def validate_consumed_final_review_recovery_receipt(
    receipt: object,
    *,
    candidate_id: str,
    grant_id: str,
    ...
    """Validate a pick-headed receipt without consulting mutable code."""
    ...
    try:
        ...
        return False
    return bool(
        actual == current_row and consumed_row.get(RECOVERY_RECEIPT_FIELD) == receipt
    )


def advance_selected_final_review_recovery_receipt(
    receipt: object,
    *,
    ...
    """Advance the exact queue/pick lineage and return a fresh self-seal."""

    if not isinstance(receipt, Mapping):
        raise ValueError("selected final-review recovery receipt is missing")
    declared_current = receipt.get("current_failure_recovery_fingerprint")
    transitions = receipt.get("transitions")
    expected_kind = (
    ...
    from_valid = (
        ...
        if expected_kind == QUEUE_TO_PICK_TRANSITION
    ...
    if not (
        ...
        and validate_selected_final_review_recovery_receipt(
            ...
            current_fingerprint=str(declared_current),
            allow_queue_rebound=(
                allow_queue_rebound and expected_kind == QUEUE_TO_PICK_TRANSITION
            ),
        )
        ...
        raise ValueError("selected final-review recovery transition is invalid")
    try:
        canonical_to = _canonical_copy(_row_body(to_row))
        next_receipt = _canonical_copy(dict(receipt))
    except (TypeError, ValueError) as exc:
        raise ValueError("selected final-review recovery transition is not JSON") from exc
        ...
        *next_receipt["transitions"],
        {
            ...
            "from_row_sha256": str(receipt.get("current_row_sha256") or ""),
...
def _current_fingerprint(candidate_id: str) -> str | None:
    ...
    except Exception:  # noqa: BLE001 - typed authority must fail closed
        return None
    return current if _valid_sha256(current) else None


def _provider_budget_disposition(
    row: Mapping[str, object],
    candidate_id: str,
    ledger_state: str,
    ledger: Mapping[str, object] | None,
) -> SelectedFinalReviewRecoveryInspection | None:
    if not provider_budget_retry_claimed(row):
        return None
    evidence = row.get("failure_evidence")
    retry = valid_retry_evidence(
        evidence.get("provider_budget_retry") if isinstance(evidence, Mapping) else None,
        candidate_id=candidate_id,
    )
    if not (
        retry is not None
        and ledger_state != LEDGER_STATE_INVALID
        and row.get("status") in {"failed", "candidate_rejected"}
        and row.get("failure_kind") == "subtitle_authority"
        and row.get("failure_stage") == "final_review_provider_budget"
        and row.get("failure_recoverable") is True
        and _valid_sha256(row.get("failure_fingerprint"))
    ):
        return SelectedFinalReviewRecoveryInspection(
            BLOCKED, "SELECTED_FINAL_REVIEW_PROVIDER_BUDGET_BLOCKED"
        )
    if ledger_state in {LEDGER_STATE_ABSENT, LEDGER_STATE_EMPTY}:
        return SelectedFinalReviewRecoveryInspection(
            OUTSTANDING, "SELECTED_FINAL_REVIEW_PROVIDER_BUDGET_OUTSTANDING"
        )
    expected = {
        "candidate_id": candidate_id,
        "retry_fingerprint": retry["retry_fingerprint"],
    }
    if ledger_state == LEDGER_STATE_CONSUMED and ledger is not None and ledger.get(
        "entries"
    ) == [expected]:
        return SelectedFinalReviewRecoveryInspection(
            CONVERGED, "SELECTED_FINAL_REVIEW_PROVIDER_BUDGET_ATTEMPT_CONSUMED"
        )
    return SelectedFinalReviewRecoveryInspection(
        BLOCKED, "SELECTED_FINAL_REVIEW_PROVIDER_BUDGET_BLOCKED"
    )
...
def inspect_selected_final_review_recovery(
    state: Mapping[str, object], *, candidate_id: str, grant_id: str
) -> SelectedFinalReviewRecoveryInspection:
    """Inspect initial or receipt-sealed v7 state without mutation."""

    blocked = SelectedFinalReviewRecoveryInspection(
        BLOCKED, "SELECTED_FINAL_REVIEW_RECOVERY_BLOCKED"
    )
    ...
    if collection in _QUEUED_TALK_COLLECTIONS:
        ...
        if ledger_state in {LEDGER_STATE_EMPTY, LEDGER_STATE_INVALID}:
            return blocked
        return SelectedFinalReviewRecoveryInspection(
            OUTSTANDING, "SELECTED_FINAL_REVIEW_RECOVERY_QUEUE_OUTSTANDING"
        )
    if collection != "picks":
        return blocked
    receipt = row.get(RECOVERY_RECEIPT_FIELD)
    if receipt is not None:
        ...
        if ledger_state in {LEDGER_STATE_EMPTY, LEDGER_STATE_INVALID}:
            return blocked
        provider_disposition = _provider_budget_disposition(
            row, candidate_id, ledger_state, ledger
        )
        if provider_disposition is not None:
            return provider_disposition
        if row.get("status") in _SUCCESS_TERMINAL_STATUSES:
            return SelectedFinalReviewRecoveryInspection(
                CONVERGED, "SELECTED_FINAL_REVIEW_RECOVERY_TERMINAL"
            )
        if row.get("status") == "media_ready_cover_pending":
            return SelectedFinalReviewRecoveryInspection(
                OUTSTANDING, "SELECTED_FINAL_REVIEW_RECOVERY_COVER_PENDING"
            )
        failure_kind = row.get("failure_kind") if isinstance(row.get("failure_kind"), str) else ""
        failure_stage = row.get("failure_stage") if isinstance(row.get("failure_stage"), str) else ""
        if (
            row.get("status") == "failed"
            and row.get("failure_recoverable") is True
            and failure_stage in _INFRA_FAILURE_STAGES.get(failure_kind, ())
        ):
            return SelectedFinalReviewRecoveryInspection(
                OUTSTANDING, "SELECTED_FINAL_REVIEW_RECOVERY_ATTEMPT_RETRY_PENDING"
            )
        if (
            row.get("failure_recoverable") is False
            and row.get("status") in _KNOWN_FAILURE_STATUSES
            and failure_stage in _DETERMINISTIC_FAILURE_STAGES.get(failure_kind, ())
        ):
            return SelectedFinalReviewRecoveryInspection(
                CONVERGED, "SELECTED_FINAL_REVIEW_RECOVERY_ATTEMPT_TERMINAL"
            )
        return blocked
    if not is_selected_final_review_rejection(row):
        return blocked
    ledger_state, _ = resolve_provider_budget_retry_ledger_history(
        row,
        candidate_id=candidate_id,
    ...
    if current == recorded:
        return SelectedFinalReviewRecoveryInspection(
            CONVERGED, "SELECTED_FINAL_REVIEW_RECOVERY_FINGERPRINT_UNCHANGED"
        )
    return SelectedFinalReviewRecoveryInspection(
        READY_TO_REQUEUE, "SELECTED_FINAL_REVIEW_RECOVERY_FINGERPRINT_CHANGED"
    )


def active_selected_final_review_recovery_scope(
    date: str,
    state: Mapping[str, object],
    candidate_ids: Collection[str] | None,
) -> tuple[str, str] | None:
    """Return the exact v7 CID/grant only after canonical admission."""
