...
- `src/autoslice/branding_intro.py`：强制谈话片头（2026-07-18 Z1 三句版「李豆沙一直是零，不对，李豆沙一直是为爱做一」）的 hash 绑定解析与成品前置拼接；中间“不对”保留原画幅，前后两句为李豆沙区域放大的 1080p 无广告画面。策略开关在 `assets/lidousha/intro/branding_intro.v1.json`，生产媒体在 `free:/opt/bilive/autoslice/assets/intro/lidousha-branding-intro.z1-budui-20260718.mp4`；歌切不加片头。
