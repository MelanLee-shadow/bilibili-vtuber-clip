...
合并稿的候选覆盖消费接口保留，但私有内容映射适配器与证明材料不随公开版分发。
普通单候选发布照常处理；遇到已声明的覆盖或后继证明而无法验证时拒绝，不把它静默改成“无覆盖”。
...
- **身份与多人场景有限。** 普通统一主播样式不是逐段身份鉴定；声纹需要单独配置。
  不要把匿名说话人编号自动绑定为真实人物。
- **研究不等于默认功能。** MOSS / MAI 局部听证与 Volcengine Flash ASR 对照工具
  不改变本版 BCUT → CPA 主线，也不代表已证明更准或更快。Flash 的 `freeze / preflight / run / score`
  使用自行提供的音频、BCUT 基线与单独封存的人工参考；真实派发须显式开启并配置凭据和预算。
  私有样本与参考稿不随仓分发，发布参考编辑距离不冒称纯声学 CER。
