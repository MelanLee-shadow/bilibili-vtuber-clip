...
- `song`: 李豆沙本人必须是整首歌的现场演唱主体。首尾及至少 80%/7 行须明确演唱；只允许一段受严格时长/比例限制的 canonical 戏剧台词。当前自动门不放行 `collab_song`：其他歌手、合唱/和声、只有其他人在唱、原唱/片尾/下播卡/游戏视频 BGM、offscreen/static replay，或李豆沙只在音乐上说话，一律不是可自动交付的李豆沙歌切。
...
- Joint performer proof before `song_complete`, recut delivery, cover work, or package materialization (LRC discovery/alignment may run first because the gate consumes actual lyric cues):
  - AGY v5 hard veto: all heard lyric rows must identify the same live Li Dousha lyric source and exclude another singer/harmony and recorded/playback vocal. The first/actual-final performed rows and at least 80%/7 performed rows must be `SINGING_THIS_LYRIC`; the only exception is one contiguous exact-lyric `PERFORMING_THIS_LYRIC_SPOKEN` block capped at 6 rows, 12s voiced, 20%, and 15s span. Three head/middle/tail evidence points must land in sung rows. Code separately derives `FULL_STUDIO_SEQUENCE` or `COMPLETE_LIVE_ARRANGEMENT`; the latter may omit only one repeated canonical block while retaining at least 8 rows/70%, 30 seconds, head/middle/tail, a global shift, actual live opening/ending, and a bound post-song transition.
  - CAM++ identity subclaim: `host-vocal-proof.v2`, `status=READY`, `decision=LIDOUSHA_VOCAL_PRESENT_ON_LYRIC_CHECKPOINTS`; its seven checkpoints must all bind `SINGING_THIS_LYRIC` rows, never spoken rows, and its source/alignment/profile/model/reference/session-anchor/checkpoint hashes and recorded medians/thresholds must verify.
...
- Automated audio proof is narrower than an ordinary model suggestion: `src/autoslice/song_repair.py` independently binds the current source and canonical LRC hashes, requires every canonical row to be classified heard/unheard at confidence `>=0.8`, validates the heard sequence's monotonic timings and one global shift, rejects unproved tempo stretch, and requires first-line/chorus/repeated-section/longest-gap/tail checks plus a host-talk or instrumental-outro boundary. `FULL_STUDIO_SEQUENCE` requires all rows. `COMPLETE_LIVE_ARRANGEMENT` permits only one repeated omitted block capped at 12 rows/30%, while retaining at least 8 rows/70%, 30 seconds, head/middle/tail, and the real live opening/ending; the rendered lyric timeline contains only performed rows. If an exact lyric repeats, `repeated_section` must point to a later audible recurrence, not its first occurrence. Search ranking or an AGY verdict alone is never proof.
- The same AGY v5 raw observation must independently classify the performance. Only `LIVE_STREAMER_SINGING` at confidence `>=0.85`, `continuous_live_song_performance=true`, background-recording likelihood `<=0.20`, exact sung-row head/middle/tail observations, and the bounded predominantly-sung contract above can pass. A studio-repeat omission by itself is not singer ambiguity; ordinary speech, `ORIGINAL_OR_BACKGROUND_PLAYBACK`, `OTHER_SINGER`, `STREAMER_TALKING_OVER_MUSIC`, true singer uncertainty, guest/duet/harmony, offscreen/static replay, missing row fields, or raw/report disagreement remains a hard veto even when LRC alignment is perfect.
- CAM++ then checks identity on the same aligned lyric evidence under `host-vocal-proof.v2`. First, 4–8 seconds of post-song host speech must score a median `>=0.60` against three pinned Li Dousha enrollments. Seven distinct actual `SINGING_THIS_LYRIC` cues, each at least 2.5 seconds, contribute a central 2.5–4 second sample; spoken rows are skipped. A checkpoint passes only when both its three-enrollment median and its comparison to the same-session host anchor are `>=0.31`; at least 5/7 plus head/middle/tail coverage is required. This only establishes `LIDOUSHA_VOCAL_PRESENT_ON_LYRIC_CHECKPOINTS`; the AGY AND is what turns it into a singing decision.
...
- AGY v5 same-subject/live-arrangement observation is missing/malformed, non-live, below confidence, discontinuous, insufficiently sung, an unbounded/non-repeat/random omission, lacks an actual live opening/ending and post-song transition, exceeds any spoken-passage cap, is too likely to be a background recording, lacks exact sung-row head/middle/tail evidence, or fails any per-row singer/role/no-replay assertion.
...
- 歌切标题铁律（Ivan 2026-07-14 定、2026-07-19 重申）：固定目录式 `【李豆沙】豆沙歌，《歌名》`，《歌名》前后不加任何字——禁止 `｜副标题`、hook 尾巴、「直播间唱」衬词。旧 "prefer hook phrasing / avoid bare catalog titles" 指导已废除，方向正好相反。
- The audit hard-blocks (`SONG_TITLE_FORMAT_INVALID`) without prefix + `《》`; the fixed catalog form passes it by construction. Enforcement is three-layer: profile template schema check (`channel_profile.py`), `title_policy.canonicalize_song_catalog_title` choke point, song lane canonical override.
...
Cover text omits `【李豆沙】豆沙歌，` — with the fixed catalog title this means song cover text is `《歌名》`, rendered big (banner-scale) per the cover skill.
...
- The overlaid cover text must omit `【李豆沙】豆沙歌，` / `【李豆沙】`; for songs that leaves `《歌名》` (the fixed catalog title minus prefix), not the full Bilibili title.
...
- Cover text is derived from the Bilibili title with the `【李豆沙】豆沙歌，` / `【李豆沙】` prefix removed (songs: `《歌名》`). If the title changes, the cover must be regenerated or restaged with matching `cover_text`.
...
- `evidence/` also includes the delivered `host-vocal-proof.v2` JSON with sung-row role bindings; the runtime proof's checkpoint WAV/hash chain remains in the protected autoslice evidence store.
