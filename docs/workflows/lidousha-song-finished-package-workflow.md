...
- Joint performer proof before `song_complete`, recut delivery, cover work, or package materialization (LRC discovery/alignment may run first because the gate consumes actual lyric cues):
  - AGY v4 hard veto: all heard lyric rows must identify the same live Li Dousha lyric source and exclude another singer/harmony and recorded/playback vocal. First/final and at least 80%/7 rows must be `SINGING_THIS_LYRIC`; the only exception is one contiguous exact-lyric `PERFORMING_THIS_LYRIC_SPOKEN` block capped at 6 rows, 12s voiced, 20%, and 15s span. Three head/middle/tail evidence points must land in sung rows.
...
- The same AGY v4 raw observation must independently classify the performance. Only `LIVE_STREAMER_SINGING` at confidence `>=0.85`, `continuous_live_song_performance=true`, background-recording likelihood `<=0.20`, exact sung-row head/middle/tail observations, and the bounded predominantly-sung contract above can pass. Ordinary speech, `ORIGINAL_OR_BACKGROUND_PLAYBACK`, `OTHER_SINGER`, `STREAMER_TALKING_OVER_MUSIC`, `AMBIGUOUS`, guest/duet/harmony, offscreen/static replay, missing row fields, or raw/report disagreement is a hard veto even when LRC alignment is perfect.
...
- AGY v4 same-subject live-performance observation is missing/malformed, non-live, below confidence, discontinuous, insufficiently sung, exceeds any spoken-passage cap, too likely to be a background recording, lacks exact sung-row head/middle/tail evidence, or fails any per-row singer/role/no-replay assertion.
