...
2026-07-12 首个片头的负例必须长期保留：两条完整的“我要为爱做零”分别位于 17:18:39 分段的 `1029900–1032560ms`、`1429160–1431020ms`，它们属于同一个非李豆沙/礼墨声纹簇，不能因文字完美匹配而进入李豆沙语料库。
...
联动来源应写成 `speaker=mixed`，并只提供逐条核验的 `trusted_ranges_ms`；范围外语句自动丢弃。不要在联动场次顶层伪造 `speaker=lidousha`。
...
- 原句：`我本来就是零，不对，我要为爱做零`
- 建议句：只有在全历史复查后仍明显减少切点才给出；当前候选为 `我本来就是零，不对，我就为爱做零`，仅把“要”改成“就”，保留反转梗。
- 两版都要交付 MP4、SRT、ASS、plan、verification 和 render manifest。
