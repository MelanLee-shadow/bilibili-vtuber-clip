...
Delivery
  ├─ no-upload review
  ├─ 维护者-authorized new-BV lane
  └─ exact same-BV lane:
     evidence-v2 template → actual final-byte review → create-only receipt
     + separate 维护者 repair authorization → durable repair state machine
...
- runner：`scripts/session_autoslice.py`
...
- package audit：`scripts/audit_review_package.py`
- final review/publish：`scripts/build_final_human_review.py`、
