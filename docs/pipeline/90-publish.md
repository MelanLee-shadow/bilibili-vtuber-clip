...
观察图也须复用 publication registry 的覆盖验证器，在读取候选包之前排除已验证的
`content_coverage` 子候选。即使查询只指定子 ID、父候选不在查询集合中，也须从当前
registry/runtime overlay 找到并重验其覆盖关系；不能仅凭 state 的 `covered_by_publication`
标签排除。输出的 `excluded_covered` 与 `excluded_published` 分开，绑定原父候选、日期和 BV，
不计第二条公开稿。已声明覆盖的证据原件缺失或漂移时，仍返回原 `PUBLICATION_REGISTRY_INVALID` 阻塞，
不伪造覆盖完成或放行；该分类不修改 registry、state、包或既有发布门。
...
合并稿还须消费 [20-selection.md](20-selection.md) 定义的已验证候选覆盖关系；仍只有原稿一个 BV，不把源候选重新当成新投稿。
