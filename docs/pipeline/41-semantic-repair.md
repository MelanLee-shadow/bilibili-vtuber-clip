...
0. **短语级重复分歧检测**（2026-07-19 看花篮 抱/帮 案）：`repetition_divergence_groups` 只抓句级复读分歧（两句 ratio≥0.72 仅差一块），抓不到「同一短语嵌在不同句式里重现、一字近音之差」（帮小李准备花篮 vs 抱小李准备那个花）。扩展：跨句 3-6 字 CJK n-gram 重复 + 单字近音差 → 自动编混淆组进裁决队列。审片员对这类的漏报是「宁缺毋滥」调参的固有残留，确定性编译器才是类解。
