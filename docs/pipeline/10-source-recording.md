...
引用扫描必须完整覆盖既定 Tier A 文字文件后，才可把一个目标判断为“未被引用”。
可选根目录确实不存在时允许跳过；目录遍历/文件读取失败、除下述已闭合目录别名外的链接/非普通类型、
文字文件超过既有20MiB扫描上限、UTF-8非法或扫描期间观察到内容/目录项身份变化时，
`cleanup_preflight_scan.py` 必须在候选/媒体规划前退出非零，不生成或刷新`--json`计划。
失败不会自动删除旧计划，后续必须消费成功的规划退出码，不能继续用未刷新的旧文件。
引用路径按实际`--base`的`out/`匹配，不得在自定义runtime中继续套用默认主机前缀。
完整性仍限于既定Tier A和文字扩展名集合及已实现的路径解析，不等于全系统引用证明；
它不取消同一runner锁/静默窗口、来源可恢复性、独立审批或执行时的目标前像检查。
普通文件/目录的前后身份检查不构成针对任意不合作同UID进程的系统隔离。

同级目录别名（例如 Python 环境的 `lib64 → lib`）只有在目标是同目录列表中的普通目录、
链接值是单个相对文件名且非自身/链式链接时才可接纳；不跟随别名，也不跳过目标内容。
目标必须在同一次正常遍历中实际扫描，链接及目标的身份、原链接值和目录项都加入最终漂移复核。
绝对/跨目录/悬空/链式目标、未实际遍历的目标、不可读或非法正文均拒绝；这不是 venv 名称白名单，
也不改变源可再生性、终态 owner、quiet-window、精确回收授权或删除执行器的任何门。

`._` 前缀不构成跳过依据。扫描器只对字节验证通过的 AppleDouble v2、FinderInfo +
空 resource-fork 双条目元数据使用二进制引用提取：校验 magic/version、唯一条目、范围、
长度和尾部，随后保留其中连续 UTF-8 路径，二进制分隔字节不拼接或静默丢弃。
同目录去掉`._`的正文必须是同次遍历中的普通文件，加入同一前后身份闭包并照常严格UTF-8
扫描。孤立旁车、未知格式、非空资源叉、坏正文或任一漂移仍阻断；不解包、不删除、不改
原字节。该读取适配不证明任意编码资源数据的引用完备，也不扩大原Tier A/路径解析范围。
