...
- 共享同一回执的合作进程，在观察器构造、每次观察及直接回执写入时，共用
  ...
  不恢复跨进程余额、不把旧回执当新预算；派发前持久化另按下一条处理。
...
- native exact-cue 的 canonical 观察器在真实 `before_request` 回调中，先扣除本次预算并
  ...
  硬退出后该预留仍保留并阻断未经对账的重新准入，不能凭重启/零进程账本返还额度；
  正常结果仍沿原单向终态封存，精确成功缓存命中不新增预留。未接此 canonical 入口的
  旧进程内消费者不据此宣称获得崩溃保护；跨进程余额恢复仍未实现，CPA与默认额度不变。
