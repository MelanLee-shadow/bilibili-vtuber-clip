...
- 已审字幕的重交付可在 spec 中声明哈希绑定的 `subtitle_redelivery_baseline`（仅 `--reuse-cover`）：源真值窗仍可改，窗外只继承旧版文本而保留本轮 BCUT 时间；新旧 cue 必须按时间强重叠一一映射。哈希漂移、漏 cue、合并/拆分或歧义映射一律拒发，禁止重烧时由随机 ASR/LLM 静默改写无关字幕。
