...
- **补入集修「合集归类错」**（Ivan 2026-07-16 报「怪兽花唄这个合集不对」）：实时公开面查得 `BV14gK36oEjN` `is_season_display=false ugc_season=None`——pi agent 手工上传走完投稿+封面返修但**漏了入集**（发布未入集=流程未完成）。现查 seasons 确认歌切合集 `小李歌唱`（season 8410735 / 正片 section **9364628**，非谈话的 9320779）→ `bili_archive_tool.py season-add`（camelCase，code 0）→ 公开面复核 `is_season_display=true ugc_season=小李歌唱(8410735)`。证据 `reports/lidousha-uploads-20260716/BV14gK36oEjN.season_add.json`。**机制缺口**：手工/单件上传路径必须把入集纳入收尾（歌切→小李歌唱），别只做投稿+封面。
...
Ivan 逐帧二审 7/10 重制版与 7/11 生产版，点名一批新错误类；全部按流水线机制修复（批2：SC锚定/插话保留/emoji剥离/礼物链；批3：SC线程接龙/自称平等/kmx仲裁/立语与流行梗语料/复读一致性/封面参考帧直通），并以最终 commit 重出 7/10 全天。侄女铁律定案 **A 无例外**（四象限枚举也改）。

### 已完成（main @ `9b45ecd`，全量 1114 passed）

- **批2 `617b0a4`**：SC 锚定仲裁（谢谢+sender 锚点→低门槛音频二选一，十麻乃两案）；念读中途插话保留（伊依案，匹配长度守卫/拼接/双自检四层打通）；表情占位剥离（；；串、生僻颜文字）。
- **礼物链 `0c711b9`（merge `9b45ecd`）**：SEND_GIFT/COMBO_SEND 入证据（kind=gift，120s 窗），谢礼物语法槽+音频仲裁钉礼物名（人鱼→流星雨案）；送礼人名被平台打码（有\*\*\*/uid=0）只记首字约束不改文本；audit 增 gift_repairs。
- **批3 `e99ad84`**：SC→弹幕**线程接龙**（同掩码前缀发送者接龙=高先验念读；谐音梗下字符相似度与音频仲裁双双结构性失效——利安/无马懿案 difflib common 仅 2——判据改为线程+紧邻窗+长度相容+弱字符护栏，全程 thread_anchored 审计）；自称平等铁律（李豆沙/小李绝不互换）；kmx↔乒乓球 confusable 组（音频强制选择）；立语=梗（非利语/日语）；「XXX是？」流行梗；复读一致性先验（睡衣案）；复读保留测试（戴上眼罩案）；`AUTOSLICE_COVER_REF_MS` 受监督封面参考帧直通（新衣服 1:22 帧待用）。
- **流行梗自动 crawler 立项**（Ivan 要求自动跟进不再人工喂）——排给专名流水线工作线，与 PSPLive 成员 crawler 同 lane。
- 部署：`af5589a` 已上线（侄女铁律三层+证据入口）；`9b45ecd` 部署已排队（等 runner 锁）。
- **7/10 全天重出**：`evals/redo-20260710-9b45ecd`（干净 state、缓存三源导入、DEV_EXCEPTION、10 分钟 timer）19:39Z 首 tick 已认领。旧 rerun-20260710-df00581 timer 已停（其 6 条谈话成品先前已拉回本地）。
- 侄女铁律定案：**A 无例外**（Ivan 原话），7/13 四象限枚举句同样改写侄女。

### 进行中

- **生产交付目录已按 Ivan 指令成为唯一权威面**（2026-07-14 凌晨）：旧代脏文件直接从生产 lidousha/<date> 删除、redo/修正件放入生产（7/10=redo 5 条整体替换；7/11 去直女/旧冷笑话+入侄女修正/冷笑话v3/MUA；7/13 入五年之约）。上传证据链不受影响（哈希冻结在已提交 manifest）。launchd 镜像已恢复运行，本地目录由它对齐生产。后续 redo 交付按此惯例：**落地即同步进生产目录**（7/11/12/13 重出收敛时执行）。
...
1. redo-7/10 出货→本地覆盖→Ivan 审质量差，再起 7/11/12/13 重出。
2. 全部收敛后：撤生产 crontab 的 `GEMINI_PAID_BACKUP_DEV_EXCEPTION=1`（恢复 ≥3 轮门）；deploy 脚本 SIGKILL 残留 DISABLED 的加固。
3. 权宜上传 10 条的 B 站替换批：用重出成品，等 Ivan 审毕点名。

## 2026-07-13：生产部署 main@df00581 + 摘 DISABLED + 三日重跑（当前）

> **18:4x UTC 增补（付费兜底提速，Ivan 口头授权）**：回填期间歌切被"生产 ≥3 完整失败轮才许付费"的门卡成龟速（每轮都在等 AGY 重置窗，生产付费 ledger 零点火）。按 Ivan「这几天密集开发可以用」授权，生产 cron runner 已加 `GEMINI_PAID_BACKUP_DEV_EXCEPTION=1`（免费链每轮仍先试、每笔付费入 `state/gemini-paid-backup/usage-*.jsonl`）。**回填（7/10-7/13）收敛后须从 crontab 移除该 env，恢复严格 ≥3 轮门。**
...
### 2026-07-14 00:4x 增补：忠实性守卫（一九零/小李两案）

- Ivan 两案同类判定：修正层无证据改写口语（190→一米九转述；生日结婚片 BCUT 误听「留下/提」、AGY 配额缺席、CPA 独裁猜成「小李」而原话是「李豆沙」）。铁律=无专名/证据依据时忠实原文。
...
## 2026-07-14 凌晨终章：审片员通病机制、拼音守卫、配额全链耗尽的两条尾巴

### 已完成

- **成品自审员**（29400f1+34a458a）：Ivan 判定成立——所有机制的发现向量都曾是他的眼睛。新层在证据车道后扫终稿（只报不改：nonword/context/self_ref/entity），同音建议自动应用（=守卫同一把尺），钦定词面(硬梗/混淆面/时效词/glossary 行首)永不自动改写（抽查当场抓住"立语→俚语"险案），其余披露进 `<cid>.review-flags.json`。审片员故障绝不熔断；`AUTOSLICE_DISABLE_FINAL_REVIEW=1` 杀开关。抽查 5 条未审成品挖出 10 处发现（从杀能力/笑林拔光/电子长颈鹿101号/翻唱的/刚能跟真猫对线等）待 Ivan 裁定。
- **拼音守卫**（687ac22）：pypinyin(0.55, 三解释器已装, 可选依赖)——同音判定=真声学等价。季下→记下/小丽→小李冤杀类根除；五年之约两句已人工真值修复重烧入库。
- 65k 输出上限+thinking_level=low+400 摘字段重试（e2ac348）；付费响应无论成败落盘取证。
- 生产 7/13 定点三条全部入库（拆解/闭环/五年，标题按 A 规则）；7/11 生日结婚第 7 跑入库。局部部署到 BASE 层完成；生产主仓部署等另一会话树净（现差 6 个 commit，纯本会话内容）。

### 进行中（含后台）

- **梦限大坏女人 + 乐队番**：全链配额耗尽（AGY 窗口、免费×3 RPD、付费 429 均 QUOTA）——今晚重跑量的代价。已停 10 分钟 tick 防烧帽，`resume-mengxianda-0710`（07:10 UTC）与 `resume-yuedui-0711`（07:20 UTC）一次性单元在免费 key 日重置(≈07:00 UTC)后自动续跑。全部机制层(34a458a)就位，配额一回即应收敛。
- redo-0712 五条 review_ready 待收敛入生产；合并版_侄女爱弯男(另一会话)在生产 7/13。

### 阻塞

- 付费 key 429：若 07:2x 续跑仍 429，需 Ivan 在 Google 控制台核对该 key 项目的配额/预算设置。
