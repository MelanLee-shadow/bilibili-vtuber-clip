...
## 8/8 深夜封面路由裁定(Ivan,受骗片封面案)

1. **砍前提通病确认为封面流水线必修项**(F-cover,原波8封面项):受骗片封面=活例
   (前提「刚被劝别再受骗」被砍,punch 独腿)。本片按 IVAN_EXPLICIT 双行重出,
   流水线修复照旧排队——单片手修不豁免通病。
2. **游戏截图封面不要求李豆沙主导画面**(Ivan 原话:「如果是截图封面,当然不要求
   李豆沙在画面里占主要部分,毕竟是游戏截图,只要截图足够有趣就行,主体肯定会是
   游戏」):source-composition 见证的 host-dominance 否决只适用于**非游戏场**;
   游戏场截图路线的判据改为"截图趣味性"(+小窗可见即可)。这收窄了现行
   「face cut/cannot become dominant subject→重绘」的适用面——witness 规则要按场景
   类型分叉,归入 F-cover 实现。
3. **F-cover-2 立项(Ivan 认可)**:小窗主体合成路线——面捕小窗表情裁出放大 +
   游戏画面背景拼贴,介于轻调与重绘之间,游戏场专用。

## 8/8 深夜追加:F12 hook 受话人归属校验(Ivan 纠错,受骗片标题案)

Ivan 指出自动 hook/标题「李豆沙**刚被劝**别再受骗」事实错误:字幕 cue1-3 的
「别再被骗」是连线主持对**上一位选手**说的(cue4「下一位,我们的08号」后李豆沙
才上场),她本人从未被劝。hook 把"场上刚发生的话"错归到主角头上——
**受话人归属**从未被 source_fact_review 校验(它验"话说过没有",不验"对谁说")。
修法=hook/标题事实审加受话人归属判项(依托说话人分离+对话结构);
波 8 队列新增 F12。同案连带:IVAN_EXPLICIT 重出的封面前提行「刚被劝别再受骗」
继承同一错误,需按 Ivan 新文案再重出——单变量重出纪律(只换文案)本身正确,
但上游事实错会穿透一切下游,事实校验必须在最上游。

## 8/9 配额裁定重申(Ivan 原话,裁定失落案)

Ivan 2026-08-09:「我记得我当时说过 88 这个 3D live 场放宽到 15 个,然后当天的
杂谈场认为是独立的,自然有 5 个」。现行代码只实现了游戏场 RESOLVED→10
(candidate_selection.py `_talk_pick_policy_for_session`),3DLive/事件场裁定
未曾落码——属裁定失落,不是执行超范围。修法(F13):事件场识别(3D回/生日回/
周年回类,标题/语境信号)→ 该场 pick 上限 15;同日多场各自独立计数(杂谈场
独立 5);带本条出处注记。落地后 8/8 候补按新政策补产。

## 8/9 F14 单人场先验(Ivan 事实结论)

Ivan:8/8 被 speaker 拒的 6 条里只有 auto_200130_1323_1603/auto_200130_1722_1792
是多嘉宾场,**其余 4 条(auto_230125_1157_1229/550_701/714_859、auto_233123_115_165)
全是李豆沙单人直播**——单人场无身份可解问题,4 条属误拒(舞台 BGM/观众声压低
