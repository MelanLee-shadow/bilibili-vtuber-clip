...
## 六之十四、8/8 谈话切实测:**0/6**,三个互相独立的拦点(groupcap 修复有效但不够)

09:22 起跑 13 条(5 并行滚动派发)。到 10:47 已完成约 6 条,**产出 `burned-final-speaker` 的:0 条**。
**但 09:12 部署之后,runner.log 里再没有出现过一次 provider failure** —— groupcap 修复**确实生效了**,
只是谈话切被别的门卡住:

| 候选 | 死法 |
|---|---|
| `auto_230125_1157_1229` | `SPEAKER_FINALIZATION_BLOCKED: not enough Li Dousha clip anchors: []` + 终审 findings |
| `auto_233123_115_165` | `SPEAKER_FINALIZATION_BLOCKED: ... anchors: [10]` + 终审 findings |
| `auto_230125_550_701` | 终审 `FINAL_REVIEW_UNRESOLVED_FINDINGS` |
| `auto_233123_315_412` | `BOUNDARY_SEMANTIC_REVIEW_REQUIRED`(实质内容判断,**这条是按设计工作**) |

**三个独立拦点**:①说话人锚点不足(对应 8/8 failure 分布里的 `speaker_evidence/speaker_finalization ×7`)
②终审正字法门(§六之六)③边界语义门(正常工作)。**我今晚的修复一个都不治这三个**——
它们治的是 provider 层,而 provider 层现在是干净的。

**结论要说清**:谈话切今晚**大概率零交付**,不是因为基础设施,而是因为内容门。
其中②有实测的解法(选项 D)但会发出已知错字,**需要 Ivan 拍板,我不越权**。

## 六之十五、把剩余时间改押歌切(调度决策)

10:47Z 我**主动 kill 掉 8/8 剩余的 3 条谈话切 produce**,让 tick 走到歌 lane。理由:
- 谈话切当前代际 **0/6**,且卡在我今晚无权放宽的内容门上;再等 90 分钟大概率仍是 0。
- 歌切是 Ivan **点名**的目标,且**不经过**上述任何一个门(`song_lane.py` 零处引用终审契约)。
- 8/8 排队的歌正是 `song_200130_1012`**《花之塔》**——**F3 跨字系修复的正主**
  (日文歌词对中文 ASR 恒 0%,此前被 4–19% 的中文同音噪声顶掉)。今晚它第一次带着 F3 上场。
- 8/9 的 5 条谈话切在 8/8 歌之后仍会排到,**没有被牺牲**(而且 8/9 是普通杂谈日,素材与 3D live 事件场不同质)。
...
## 九、⭐ 歌切真凶找到并已修复部署(Ivan 追问后的第二轮)

Ivan 追问「那你能不能修复一下」→ 深挖后**真凶收敛到一条**,已修复并部署 **`bf0d008`**(14:01:59Z,3773 绿)。

### 真凶:会话锚点搜索视野人为过窄
`song_terminal_disposition.reason_codes` **只有** `SONG_NOT_LIDOUSHA_SINGING`,其余全是下游噪声。
声纹失败的直接原因不是素材差,而是:
- 锚点(桥接"说话 enrollment ↔ 唱歌")只从 `post_song_talk_start_ms` 起搜 **48 秒**;
- 本例起点 `258000ms` **根本不是说话**,是这场联唱的**下一首歌**(SRT 269.94–308.69s 是罗马音日语歌词);
- 真说话从 **486.04s**「欢迎回来」起,**视野尾端 306000ms 离它还差 180 秒**;
- 13 个候选全扫在唱歌上,最高 **0.30084** < `MIN_SESSION_ENROLL_MEDIAN=0.50`
  → 桥接通道整个关闭 → 检查点只能硬碰说话声纹 → **1/7**。

### 实验证据(先证可比,再下结论)
控制组复算部署 proof 里 3 个候选,**逐位精确、差 0.0**。全长 602.136s / 150 窗:
| 区段 | 最高分 | ≥0.50 窗数 |
|---|---|---|
| pre_song | 0.27512 | 0 |
| in_song | 0.31466 | 0 |
| **post_song** | **0.78569** @548000–556000 | **27**(488000→源尾连续) |
| 部署 48s 视野内 | 0.30084 | 0 |

**并否掉替代假设**:本场 0.78569 **高于** 7/25 已发布成功件《海海海》的 0.57341,
所以不是"3D live 音频链压低相似度",是搜索窗压根没扫到。

### 修法(阈值一字未动)
搜索扫到 source 末尾;显式排除与 hash-bound 歌词区间重叠的窗
(纵深防御:防"用她的演唱验证她的演唱"给播放原唱开口子)。
`0.50 / 0.31 / 5-of-7 / 0.22` 与 `SINGING_THIS_LYRIC` 全部原样。
**只搜歌后、不扩到歌前**——这是 worker 驳回 integrator 倾向后采纳的取舍:
新候选序列是旧序列的**严格超集且前缀相同**,7/25 已发布件重出证会选到**同一锚点**;
扩到歌前会改变"第一个通过的窗",等于**悄悄改掉一件已发布成品的证据面**。
新增 4 测试(3 条修复前实测为红;第 4 条循环守卫前后皆绿,**worker 主动如实标注未包装成红转绿**)。

### 已复活,等下播重跑
`song_210131_1210` 已用 `revive_rejected_candidates.py --lane song --apply` 复活:
`candidate_rejected → blocked`、17 条陈旧码撤回、终态裁定清空、
`song_pipeline_fingerprint = sanctioned-revival:bf0d008`(哨兵保证会被重排)、审计块 `song_revivals` 已落。
**只读预测**:第一个通过窗 490000–498000ms / 0.55836 → 7 个检查点 **7/7 通过 → READY**。
**这是预测,真 proof 必须由 runner 重出证。**

### ⚠️ 同源第二缺陷(**未修,留给 Ivan**)
`post_song_talk_start_ms` 由 AGY(本次 rc=-9 挂掉)回退到 `gemini-3.6-flash` 观察得出,
**在联唱/3D live 连续歌单场景把"歌间间隙"当成 `HOST_TALK`**;
且 `song_performance.py:292` 强制它与 `post_song_transition_ms` 相等,**两字段一起错,交叉校验查不出**。
它还在喂 `recut_materialization.py:1127` 与边界收紧。放宽搜索面**绕过**了它,**没有修正**它。

### 剩余风险
1. 出证变慢:成本 ≈ `(歌后毫秒/4000)×3.94s`;**歌后尾巴超约 15 分钟会撞 prover 900s 超时**。
   治本是让 `generate_host_vocal_proof` 也走 `speaker_finalizer` 的 embed-once(实测差 ≤4.4e-6),未做。
2. **21 份现存 proof 失去可复验性**(policy 是 hash-bound),下次 tick 重出证;
   READY 件结论与锚点不变,BLOCKED 件应当失效(13 窗不再构成穷尽证明)——fail-closed 正确方向。

### 我这一轮犯的两个错(记账)
1. **只搜 free 就下"从来没有"的全称否定**——被 Ivan 一句话推翻(§六之十八 更正声明)。
2. **把自持锁的脚本套进外层 `flock`** → 自锁死,3 个 revive 进程卡住。
   memory 对 deploy 记过同样的坑,我没类推。`revive_rejected_candidates.py` **自己持 `runner.lock`,不要再套 flock**。

## 七、留给 Ivan 的待裁项(未擅自决定)

**按重要性排序。前两条会直接决定谈话切能不能量产。**

### A. 终审正字法门:选 B 还是 D?(**最要紧**)
实测表见 §六之六。一句话:
- **B**(白名单补 2 个 keep-current 分支名)= **纯修 bug**,不削弱任何纪律,但**救不了当下**(今天 0/3)。
- **D**(所有 `repaired=True` 一律保留原文)= **解锁 5/5、3/3**,但会把审片员**已判定为错**的字幕原样发出去
  (`我去，张死` / `烧哦` / `刚刚香香烧烤说` / `超超级紧张`)。编造风险零,**已知错字上线风险满**。
- 更深的病:终审**根本没有 apply 通道**(`final_review_auditor.py:2644` 把改好的 SRT 直接丢弃),
  所以"判定该改"的 finding **永远回不到 resolved**。真正的根治是补 apply 通道,不是在白名单上打补丁。
- 附:该门署名的「Ivan 2026-07-26/27 无人值守裁定」**查无本人 user turn**(第 4 例疑似伪裁定)。
  若那三个分支名不是你划的线,**B 就只是修 bug**,我可以直接做。

### B. sudocode 两个分组的路由修复(你的 #8)—— 根治在你的 oracle
今晚实测:分组抽签 **每请求 15–17% 失败**、与大小/模型无关(§六之八)。
我只在客户端把它当瞬时故障重试(已部署 `c6a323f`),**没碰你的 oracle**。
根治后这条客户端缓解可以保留(它也顺带覆盖真正的瞬时抖动)。
另:`gpt-5.6-sol` 今晚一度 **6/6 返回 503 `auth_unavailable (providers=codex)`**,是凭据/配额面,与分组是两回事。

### C. `--preclaim` CLI 的毁 state 缺陷(§六之九)—— 建议尽快修
在任何**已有内容**的日期上跑一次就会抹掉当天全部 picks/已发布台账。建议改成合并写,或加"目标 state 非空即拒"的门。

### D. `visual_song_discovery` 的 fail-open(§六之七)
接触表超 20MB 上限即失败,却 **fail-open 静默读成"这场没有歌"**。今晚不阻塞(候选已在 backlog),
但 AGY 一 OOM 就会让歌切**静默归零而不是报错**。建议分批/降采样 + 超限 fail-closed。

### E. 1323 合集分P标题(你睡前说「合集补标题不用等吧？」)
契约已离线查清(§六之五之外,详见 `docs/reviews/2026-08-10-season-episode-edit-contract.md`):
`sorts[].id` 要**合集内单集 id**(不是分P cid),且必须发**整节全表 103 条**——即一次**整节重排写**。
我**没有在活稿件上再试**(你的原话:不在活稿件上试错)。两条路请你选:
①按新契约跑一次——**注意:修正后的 `season_episode_edit` 已合入 Mac 分支尖但尚未部署**(`tmp-section-title` → main,3769 绿),所以走这条路要**先部署**;另需确认没有上传在飞,因为 free 每传完一条就会往同 section 做 `episodes/add`,存在竞态;
②你在创作中心手改那一行(最省事)。
另:仓里那两处"期望 payload"的测试与实现是**同一个 commit `6243dfb` 自证**的,认证了一个从未成功过的请求体。

### F. 其余
1. 多嘉宾场「可交付但依赖审阅」政策 —— 未落地。
2. 8/7 配额追认争议(4 条 89.0/87.25/86.75/86.0 在 90 门下不合格)—— 仍待裁。
3. `scripts/import_external_package.py` 已交付并部署,但**今晚不是瓶颈**(上传配额才是);
   其真实价值是省掉 integrator 的人工六步。**首次真实调用务必先 dry-run**(manifest/audit/QC 三步只在打桩 seam 上验过)。
4. **8/7 目前是 `manual_preclaim`**(我为让产能给 8/8/8/9 而暂停),记得按需解除——
   **解除请用逐字段改法,不要用 `--preclaim` CLI**(见 C)。
5. `auto_210739_1142_1436`(8/7,8.17 分名场面)审计只差 punch v2 回执,
   修法是**只重生成封面**;但 8/7 现在被我暂停,runner 不会自动做。

---

## 八、终局(8/9 五条的结果 + 交棒状态)

**8/9 五条于 11:23:25 全部并行起跑**(`cap=5 slots=5 kept=5 出处=asset:default_policy`):
`auto_214238_835_960` / `auto_200615_1320_1467` / `auto_193611_1250_1450` /
`auto_193611_1612_1693` / `auto_190617_473_766`(置信度 0.88–0.96)。
...
### 我改了但**没有**部署的东西
无。今晚所有合入都已部署(`c6a323f`),工作树干净,分支 `claude/session-live-context`。

### 未合入的分支(内容已在别处或待你裁定)
- `tmp-orthography`:正字法门诊断报告(**只有报告,零代码改动**),等你在 §七A 里选 B 还是 D。
- `tmp-song-fix-f1f3`(另一会话的):内容与我已合入的 `016d889` 等价,保留未动。
- `tmp-j2` / `tmp-f20`:内容已在 `a2b07e8` 里,是残留,可删。
