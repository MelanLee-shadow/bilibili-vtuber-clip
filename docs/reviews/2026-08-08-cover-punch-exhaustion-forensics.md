...
但缺口在于：路由从未尝试过把"我就是女同"单独作为唯一一行（`main` 独占、`sub=null`），
也没有尝试保留"如果是我杀的"因果结构的更短同义抽取（如"她说如果是我杀的"这类更短的
条件从句变体）；候选生成器目测只在"标题里的几个名词短语"之间排列组合，没有对"必须带
因果连接词才成句"这个反馈做针对性调整——这是可改进但非致命的路由局限。
