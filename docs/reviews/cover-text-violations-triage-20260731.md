...
无梗字的短文案目前锁单行，窄边栏（左右分栏 zone 约 700px）下 8 字要到 120px 下限需要
~960px，装不下即阻断并转梗字评审重跑。梗字封面本来就强制 banner 全宽文字区
（`_punch_layout_override`，2026-07-20 生态调研：窄边栏 9 字只有 ~82px，banner 下 165px）。

**是否让 no-punch 短文案也一律走 banner**，是 Ivan 的审美辖区，本次没有动——
版面家族变更不应作为 bug 修复的副产品静默发生，且它必须在艺术指导阶段连同 CPA 构图
prompt 一起改（overlay 阶段换区会把文字压到已按分栏生成的人物上）。
