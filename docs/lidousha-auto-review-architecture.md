...
> **当前状态纠正（2026-07-10）**：本文保留了 2026-06/07 分阶段建设记录；下文“日语歌词只有风险标记 / 自动 LRC 对齐未实现 / source-context 尚未串入歌词证明”等句子是当时的历史快照，不再代表现状。当前 song lane 会把 upstream seed anchor 带过 tight/core/full selector，`--lrc-provider auto` 使用 NetEase+LRCLIB+Kugou；稀疏/乱码日语唱歌 ASR 在唯一 canonical LRC 身份成立后，可由 expanded full retry 对**当前音频 + canonical LRC**运行 hash-bound AGY High 对齐。《芽吹くとき》v4 只证明这条 LRC/边界路径，却实际是下播卡播放原曲，已 superseded/rejected。当前源码增加 AGY v3 逐歌词行“李豆沙本人、正在唱该行、同一现场声源、无其他歌手/和声、无预录/回放人声”硬否决 AND CAM++ `LIDOUSHA_VOCAL_PRESENT_ON_LYRIC_CHECKPOINTS`；只有两者 AND 才可 `VERIFIED_LIDOUSHA_SINGING`。歌段 source interval 还会持久隔离所有重叠 talk 候选，禁止 lane laundering。新联合门的生产部署/fresh 重跑尚待 live acceptance；精确证据见 `docs/reviews/2026-07-10-lidousha-host-vocal-song-gate.md`、`docs/reviews/2026-07-09-mebukutoki-lrc-repair.md` 和 `docs/autoslice-capability-status.md`。歧义/错版/无同步 LRC/非现场演唱/声纹/渲染证明任一失败仍 BLOCK；本能力不启用上传。
