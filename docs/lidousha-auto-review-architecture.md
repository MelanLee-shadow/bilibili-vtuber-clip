...
> **当前状态纠正（2026-07-10）**：本文保留了 2026-06/07 分阶段建设记录；下文“日语歌词只有风险标记 / 自动 LRC 对齐未实现 / source-context 尚未串入歌词证明”等句子是当时的历史快照，不再代表现状。当前 song lane 会把 upstream seed anchor 带过 tight/core/full selector，`--lrc-provider auto` 使用 NetEase+LRCLIB+Kugou；稀疏/乱码日语唱歌 ASR 在唯一 canonical LRC 身份成立后，可由 expanded full retry 对**当前音频 + canonical LRC**运行 hash-bound AGY High 对齐。《芽吹くとき》historical v4 只证明这条 LRC/边界路径，却实际是下播卡播放原曲，已 superseded/rejected。当前生产 commit `f64cd29` 已部署 AGY v4 AND `host-vocal-proof.v2` 联合门：AGY 要求首尾演唱、至少七行且至少 80% 演唱，只允许一个受六行/12 秒/20%/15 秒四重限制的歌曲内 `PERFORMING_THIS_LYRIC_SPOKEN` 戏剧对白块，且三段 evidence 必须落在演唱行；其他歌手/和声、录制/回放人声、普通说话+BGM、静态/离屏 replay 一律硬否决。CAM++ 七个 checkpoint 只从 `SINGING_THIS_LYRIC` 行选取，v2 重新绑定 required role 与 subject/role/same-source/no-other/no-recorded 五项断言；只有 AGY-live AND CAM++ identity 才可 `VERIFIED_LIDOUSHA_SINGING`。歌段 source interval 仍持久隔离所有重叠 talk 候选，禁止 lane laundering。事故六文件修复已 `COMMITTED`；真唱《屑屑》v5 no-upload 正例已 `READY/MATERIALIZED`，同音轨静态 replay 被 AGY 阻断，fresh《芽吹くとき》自动找到正确 LRCLIB 日文 LRC 并 25/25 对齐后，本轮 AGY 虽误报 live，仍由 `host-vocal-proof.v2` 0/7 最终 `BLOCK / SONG_NOT_LIDOUSHA_SINGING`。10:50Z exact cron smoke 返回 0 且 state/summary/upload ledger 不漂移，`DISABLED` 已移除。精确证据见 `docs/reviews/2026-07-10-lidousha-host-vocal-song-gate.md`、`docs/reviews/2026-07-09-mebukutoki-lrc-repair.md` 和 `docs/autoslice-capability-status.md`。歧义/错版/无同步 LRC/任一联合门/渲染证明失败仍 BLOCK；本能力不启用上传。
