...
OUT = Path('/app/reports/auto_review_shadow/backtest-20260619-manual-seeded-300s-01')
CANDIDATE_ID = 'manual_seed_300500_411000'
MANUAL = {'start_ms': 315000, 'end_ms': 438000}
...
def main() -> None:
    ...
    seed_start = 300_500
    seed_end = 411_000
    seed_cues = [cue for cue in cues if cue.source_end_ms > seed_start and cue.source_start_ms < seed_end]
    ...
    request = CpaSemanticQaRequest(
        ...
        terminology=CpaTerminologyContext(
            ...
            applied_terms=('kmx',),
            evidence_paths=('/app/lidousha/term_lexicon.json',),
        ),
        metadata={'mode': 'backtest-seeded-from-20260619-slice_candidates', 'manual_reference': MANUAL},
    )
    ...
    job = {
        'schema_version': 'source-context-job-from-manual-backtest-seed.v1',
        'candidate_id': CANDIDATE_ID,
        'title': normalized_text[:80],
        'room_id': '22966160',
        'timeline': {
            'source_duration_ms': source_duration_ms,
            'anchor_start_ms': seed_start,
            'anchor_end_ms': seed_end,
            'context_start_ms': seed_start,
            'context_end_ms': seed_end,
            'context_duration_ms': seed_end - seed_start,
        },
        'provenance': {
            'seed_source': 'official_source slice_candidates selected[0] hybrid candidate',
            'manual_reference_start_ms': MANUAL['start_ms'],
            'manual_reference_end_ms': MANUAL['end_ms'],
        },
    ...
    tl = (record.get('source_context_job') or {}).get('timeline') or {}
    recut = record.get('materialized_recut') or {}
    result = {
        'out': str(OUT),
        'manual_reference': MANUAL,
        'seed_window_ms': {'start_ms': seed_start, 'end_ms': seed_end},
        'candidate_text': candidate_text,
        'normalized_text': normalized_text,
    ...
    (OUT / 'manual_seed_backtest_result.json').write_text(json.dumps(result, ensure_ascii=False, indent=2, sort_keys=True) + '\n', encoding='utf-8')
